...
void loadplayerhome(CHAR_DATA *ch);
...
inet_aton("204.209.44.3", &sa.sin_addr);
...
/*    int iLang;*/
	...
	if ( !d->character )
	{
	    ...
	    log_string( log_buf );
	    write_to_buffer( d, "Your playerfile is corrupt...Please notify databat@crud.crodo.com.\n\r", 0 );
	    close_socket( d, FALSE );
	    return;
	}
	ch   = d->character;
...
//            sprintf( buf, "Did I get that right, %s (Y/N)? ", argument );
	...
	ch = d->character;
        if(!str_cmp(ch->name, "Arcturus") );
   	else if ( ch->top_level > 100 )
	        talk_channel(ch, "&wTears forth from the heavens and enters the galaxy.",CHANNEL_OOC,"&cconnect&r:");
	else
	        talk_channel(ch, "&Wenters the galaxy. &r[connected]",CHANNEL_OOC,"&cconnect&r:");
	to_channel( ch->name,CHANNEL_MONITOR,"Monitor",ch->top_level);
	if ( ch->top_level < LEVEL_DEMI )
	{
...
for ( iRace = 0; iRace < (MAX_RACE) ; iRace++ )
	...
	write_to_buffer( d, "\n\rPlease choose a main ability from the folowing classes\n\rNote that choosing an invalid class will default to combat.\n\rThe classes are... [", 0 );
	buf[0] = '\0';
    ...
    if ( iClass == FORCE_ABILITY )
        {
            write_to_buffer( d,
                "This class is currently disabled\n\rPlease choose a valid class!\n\r", 0 );
            return;
        }


/*
    if ( iClass >= MAX_ABILITY -3
...
*/
	...
	case 'n': case 'N': 
	    ...
	    ch->perm_int = number_range(1, 6)+number_range(1, 6)+number_range(1, 6);
	    ch->perm_wis = number_range(1, 6)+number_range(1, 6)+number_range(1, 6);
	    ch->perm_dex = number_range(1, 6)+number_range(1, 6)+number_range(1, 6);
	    ch->perm_con = number_range(1, 6)+number_range(1, 6)+number_range(1, 6);
	    ch->perm_cha = number_range(1, 6)+number_range(1, 6)+number_range(1, 6);
...
*/
    ...
    case CON_READ_MOTD:
	write_to_buffer( d, "\n\rWelcome to Star Wars Life...\n\r\n\r", 0 );
	add_char( ch );
	d->connected	= CON_PLAYING;
        
        if ( !IS_NPC(ch) && IS_SET( ch->act , PLR_SOUND ) )
           send_to_char( "!!SOUND(starwars V=100)" , ch );
	...
	if ( ch->top_level == 0 )
	{
	    ...
	    pcnt1 = number_percent();
	    pcnt2 = number_percent();
	    pcnt3 = number_percent();
	    pcnt4 = number_percent();



   	    sprintf( log_buf, "New Character mainability - %d (%d,%d,%d,%d).", ch->main_ability, pcnt1, pcnt2, pcnt3, pcnt4 );	
            log_string_plus( log_buf, LOG_COMM, UMAX( sysdata.log_level, ch->top_level ) );
...
/* Arcturus Random Force Roll */
            if (ch->main_ability != HUNTING_ABILITY && ch->race != RACE_DROID)
	    {
       	      if (pcnt1 <= 15)
              { 
	...
	if (ch->perm_frc >= 1)
	    ...
	    /* took out automatically knowing common
...
bool check_parse_name( char *name )
{
    ...
    if ( is_name( name,
      ...
      "luke leia public Rebellion The-Empire the-gods Corp-Auth Sith-Clan Blacksun Hunters-Guild Merr-Son-Inc."
      "han yoda bobo booboo kelten felten assmunch cocksucker yousuck slayme fuckyou new-republic wedge wedgie empire" 
      "the dick cock asshole biggs grey-council corp-authority corporate darklighter darth sidious vader spam" ) )
	return FALSE;
...
bool check_multi( DESCRIPTOR_DATA *d , char *name )
{
        ...
        && !str_cmp(dold->host , d->host ) )
	{
	        ...
	        if ( get_trust( d->character ) >= LEVEL_NEOPHYTE 
		|| get_trust( dold->original ? dold->original : 
dold->character ) >= LEVEL_NEOPHYTE )
		    return FALSE;
...
char *default_prompt( CHAR_DATA *ch )
{
  ...
  if (ch->skill_level[FORCE_ABILITY] > 1 || get_trust(ch) >= LEVEL_IMMORTAL )
    strcat(buf, "&BF&borce&z:&c%m&z/&C%M  &BA&blign&z:&c%a\n\r");      
  strcat(buf, "&BH&bealth&z: &c%h&z/&C%H &BM&bovement&z: &c%v&z/&C%V &z(&c%g&z/&C%G&z)");
  strcat(buf, "&c >&w");
  return buf;
}
      ...
      else /* ok! */
       {
          write_to_descriptor (desc, "\n\rWarmboot recovery complete.\n\r",0);
           ...
           if (!d->character->in_room)
                d->character->in_room = get_room_index (ROOM_DEFAULT_CRASH);
           /* Insert in the char_list */
           loadplayerhome(d->character);
