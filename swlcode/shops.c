...
long	get_cost	args( ( CHAR_DATA *ch, CHAR_DATA *keeper,
...
long get_cost( CHAR_DATA *ch, CHAR_DATA *keeper, OBJ_DATA *obj, bool fBuy )
{
    SHOP_DATA *pShop;
    long cost;
    bool richcustomer;
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "You sell $p for %ld credit%s.",
	cost, cost == 1 ? "" : "s" );
...
void do_value( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "$n tells you 'I'll give you %ld credits for $p.'", cost );
                 ...
                 char *arg, int maxgold, char *fixstr, char*fixstr2 )
{
   ...
   {
      sprintf( buf,
       "$N tells you, 'It will cost %ld credit%s to %s %s...'", cost,
   ...
   else
   {
      ...
      sprintf( buf, "$N charges you %ld credit%s to %s $p.",
...
void appraise_all( CHAR_DATA *ch, CHAR_DATA *keeper, char *fixstr )
{
    ...
    {
        if ( obj->wear_loc  == WEAR_NONE && can_see_obj( ch, obj )&& ( obj->item_type == 
ITEM_ARMOR ||   obj->item_type == ITEM_WEAPON ||   obj->item_type == ITEM_DEVICE  || obj->item_type == ITEM_CARMOR ) )

	{
            ...
            {
            sprintf( buf,
            "$N tells you, 'It will cost %ld credit%s to %s %s'",
            cost, cost == 1 ? "" : "s", fixstr, obj->name );
            act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
            total += cost;
            }
	}/*end the big ass if that started it all*/
    }
    if ( total > 0 )
    {
       send_to_char ("\n\r", ch);
       sprintf( buf,
          "$N tells you, 'It will cost %ld credit%s in total.'",
          total, cost == 1 ? "" : "s" );
       act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
       strcpy( pbuf,
...
void do_appraise( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
       "$N tells you, 'It will cost %ld credit%s to %s that...'", cost,
       cost == 1 ? "" : "s", fixstr );
    act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
    if ( cost > ch->gold )
      act( AT_TELL, "$N tells you, 'Which I see you can't afford.'", ch,
