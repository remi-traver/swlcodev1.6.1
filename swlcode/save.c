...
//    fprintf( fp, "DC           %d\n",	ch->damcap          	);
    fprintf( fp, "DR           %d\n",	ch->damage_resistance	);
    fprintf( fp, "Fstyle       %d\n",	ch->fstyle		);
    fprintf( fp, "MR           %d\n",	ch->magic_resistance	);
    fprintf( fp, "CF           %d\n",	ch->forsaken	 	);
    fprintf( fp, "CU           %d\n",	ch->canultimate	 	);
    fprintf( fp, "ID10T        %d\n",	ch->stupid	 	);
    fprintf( fp, "BIRTHDAY     %d %d %d\n", ch->pcdata->birthday.year, ch->pcdata->birthday.month, ch->pcdata->birthday.day);
    fprintf( fp, "AGE          %d\n", ch->pcdata->age           );
	if (IS_IMMORTAL(ch))
{
	    fprintf( fp, "CB           %d\n",	ch->blessed	 	);
    ...
    {
    fprintf( fp, "LastMset         %s~\n",	ch->lastmset		);
    fprintf( fp, "LastAdvance         %s~\n",	ch->lastadvance	);
	...
	if ( file_ver > 1 )
	{
	  for ( i = 0; i < MAX_WEAR; i++ )
		...
		if ( save_equipment[i][x] )
		{
		    ...
		    break;
	}/*for the if ( file_ver > 1 ) if statement*/
    }
...
void fread_char( CHAR_DATA *ch, FILE *fp, bool preload )
{
    ...
    time_t lastplayed;
    int sn, extra;
    /* Preassign age to catch people without birthdays.*/
    ch->pcdata->birthday.year = -1;
	...
	case 'A':
		...
		if ( x0 >= 0 && x0 < MAX_ABILITY )
		{
		      ...
		      ch->experience[x0] = x2;
                ...
                {
                    bug("Fread_char: Unknown Ability\n\r", 0);
                }
		fMatch = TRUE;
		break;
	    }
	...
	case 'B':
	    ...
	    KEY( "Blood",	ch->blood,	fread_number( fp ) );
            if ( !str_cmp( word, "BIRTHDAY") )
            {
                line = fread_line( fp );
                x0=x1=x2=0;
                sscanf( line, "%d %d %d", &x0, &x1, &x2 );
                ch->pcdata->birthday.year = x0;
                ch->pcdata->birthday.month = x1;
                ch->pcdata->birthday.day = x2;
                fMatch = TRUE;
                break;
            }
	    break;
	...
	case 'C':
	    ...
	    KEY( "CU",	ch->canultimate,		fread_number( fp ) );
	    KEY( "CB",	ch->blessed,		fread_number( fp ) );
	    KEY( "ClanLevel",	ch->pcdata->clanlevel,		fread_number( fp ) );
	    KEY( "CF",	ch->forsaken,			fread_number( fp ) );
	...
	case 'D':
	    ...
	    KEY( "DR",	ch->damage_resistance,		fread_number( fp ) );
//	    KEY( "DC",	ch->damcap,		fread_number( fp ) );
	    KEY( "Description",	ch->description,	fread_string( fp ) );
            KEY( "Disguise",  ch->pcdata->disguisename, fread_string( fp ) );
	...
	/* 'E' was moved to after 'S' */
	    ...
	    KEY( "Fstyle",	ch->fstyle,	fread_number( fp ) 
);
	...
	case 'I':
	    ...
	    KEY( "ID10T",	ch->stupid,		fread_number( fp ) );
	    break;
	...
	case 'L':
	    ...
	    KEY( "LastPaid",	ch->lastpaid,		fread_number( fp ) );
	    KEY( "LastMset",	ch->lastmset,		fread_string( fp ) );
	    KEY( "LastAdvance",	ch->lastadvance,		fread_string( fp ) );
	...
	case 'M':
	    ...
	    KEY( "MR",      ch->magic_resistance,       fread_number( fp ) );
	...
	case 'N':
	    KEY( "NH",	ch->newbiehelper,		fread_number( fp ) );
	...
	case 'S':
	    ...
	    KEY( "Susceptible",	ch->susceptible,	fread_number( fp ) );
            ...
            if ( !str_cmp( word, "StyleSkill" ) )
	    {
		line = fread_line( fp );
		x1=x2=x3=x4=0;
		sscanf( line, "%d %d %d %d",
		      &x1, &x2, &x3, &x4 );
		ch->defensive_skill = x1;
		ch->evasive_skill = x2;
		ch->offensive_skill = x3;
		ch->agressive_skill = x4;
		fMatch = TRUE;
		break;
	    }
	...
	case 'E':
		...
		if (!ch->pcdata->bio)
		  ch->pcdata->bio	= STRALLOC( "" );
		if (ch->pcdata->wizinvis >= 106)
		  ch->pcdata->wizinvis = 1;
                if(!ch->pcdata->disguisename)
                {
                   if(IS_SET(ch->pcdata->flags, PCFLAG_DISGUISED))
...
/***********
Don't know who the hell added this -- but it's covered in update.c.

-Raijen
		if ( !ch->in_room )
		    ch->in_room =ROOM_VNUM_TEMPLE;
*********************/
...
//		  ch->damcap = 0;
		...
		if (!ch->evasive_skill)
		  ch->evasive_skill = 0;
		if (!ch->offensive_skill)
		  ch->offensive_skill = 0;
		if (!ch->agressive_skill)
		  ch->agressive_skill = 0;
		if (!ch->defensive_skill)
		  ch->defensive_skill = 0;
		if (!ch->pcdata->clanlevel || ch->pcdata->clanlevel < 1)
		...
		if (!ch->pcdata->rank)
		  ch->pcdata->rank	= str_dup( "" );
		if (!ch->damage_resistance == 0 )
		  ch->damage_resistance = 0;
	...
	case 'T':
	    ...
	    KEY( "Trust", ch->trust, fread_number( fp ) );
            /* Let no character be trusted higher than avatar - Raijen */
//	    ch->trust = UMIN( ch->trust, 100 );
