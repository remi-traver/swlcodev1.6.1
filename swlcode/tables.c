...
//    if ( !str_cmp( name, "spell_faerie_fire" ))	     return spell_faerie_fire;
    ...
    if ( !str_cmp( name, "spell_frostball" ))	     return spell_frostball;
    ...
    if ( !str_cmp( name, "spell_research" ))	     return spell_research;
    ...
    if ( !str_cmp( name, "spell_darkmatter" ))     return spell_darkmatter;
    ...
    if ( !str_cmp( name, "spell_disabled" ))      return spell_disabled;
...
DO_FUN *skill_function( char *name )
{
	...
	if ( !str_cmp( name, "do_abdicate" ))		return do_abdicate;
	if ( !str_cmp( name, "do_addally" ))		return do_addally;
	...
	if ( !str_cmp( name, "do_allpolotics" ))	return do_allpolotics;
	...
	if ( !str_cmp( name, "do_antirestore" ))		return do_antirestore;
	if ( !str_cmp( name, "do_arch" ))		return do_arch;
	...
	if ( !str_cmp( name, "do_availships"))		return do_availships;
	...
	if ( !str_cmp( name, "do_besiege" ))		return do_besiege;
	...
	if ( !str_cmp( name, "do_cheap" ))		return do_cheap;
	...
	if ( !str_cmp( name, "do_compare" ))		return do_compare;
        if ( !str_cmp( name, "do_concentrate" ))        return do_concentrate;
	...
	break;
    case 'd':
   if ( !str_cmp( name, "do_dbreath" ))    return do_dbreath;
   if ( !str_cmp( name, "do_disabled" ))    return do_disabled;
   if ( !str_cmp( name, "do_decwar" ))     return do_decwar;
	if ( !str_cmp( name, "do_decpeace" ))		return do_decpeace;
	if ( !str_cmp( name, "do_delclan"))		return do_delclan;
	if ( !str_cmp( name, "do_deleteship"))		return do_deleteship;
	...
	if ( !str_cmp( name, "do_docking" )) 		return do_docking;
	...
	if ( !str_cmp( name, "do_echo" ))		return do_echo;
        if ( !str_cmp( name, "do_elite_patrol" ))       return 
do_elite_patrol;
	...
	if ( !str_cmp( name, "do_flee" ))		return do_flee;
        if ( !str_cmp( name, "do_flushpoison" ))          return do_flushpoison;
	...
	if ( !str_cmp( name, "do_forsake" ))		return do_forsake;
	...
	break;
    case 'g':
	if ( !str_cmp( name, "do_gecko" ))		return do_gecko;
	...
	break;
    case 'i':
	if ( !str_cmp( name, "do_i100" ))		return do_i100;
	...
	if ( !str_cmp( name, "do_i106" ))               return do_i106;
	if ( !str_cmp( name, "do_i107" ))               return do_i107;
	if ( !str_cmp( name, "do_ide" ))		return do_ide;
//	if ( !str_cmp( name, "do_immpromote" ))		return do_immpromote;
//	if ( !str_cmp( name, "do_immdemote" ))		return do_immdemote;
	...
	if ( !str_cmp( name, "do_immtalk" ))		return do_immtalk;
        if ( !str_cmp( name, "do_inoculate" ))          return do_inoculate;
	if ( !str_cmp( name, "do_iooc" ))		return do_iooc;
	if ( !str_cmp( name, "do_infochan" ))		return do_infochan;
	...
	if ( !str_cmp( name, "do_interdict" ))		return do_interdict;
	...
	if ( !str_cmp( name, "do_light" ))		return do_light;
        if ( !str_cmp( name, "do_linklaser"))           return do_linklaser;
	...
	if ( !str_cmp( name, "do_loadarea" ))		return do_loadarea;
        if ( !str_cmp( name, "do_loadinoculator" ))     return do_loadinoculator;
	...
	if ( !str_cmp( name, "do_look" ))		return do_look;
        if ( !str_cmp( name, "do_loremedicine" ))       return do_loremedicine;
	...
	if ( !str_cmp( name, "do_makebludgeon" ))	return do_makebludgeon;
	if ( !str_cmp( name, "do_makedoublelightsaber" )) return do_makedoublelightsaber;
	...
	if ( !str_cmp( name, "do_makejewelry" ))	return do_makejewelry;
        if ( !str_cmp( name, "do_makeforcepike"))       return do_makeforcepike;
	if ( !str_cmp( name, "do_makeboard" ))		return do_makeboard;
	if ( !str_cmp( name, "do_makeclan" ))		return do_makeclan;
        if ( !str_cmp( name, "do_makeinoculator" ))     return do_makeinoculator;
	...
	if ( !str_cmp( name, "do_makeshop" ))		return do_makeshop;
        if ( !str_cmp( name, "do_makeshot" ))           return do_makeshot;
	...
	if ( !str_cmp( name, "do_mp_restore" ))		return do_mp_restore;
        if ( !str_cmp( name, "do_mp_refresh" ))         return do_mp_refresh;
	...
	if ( !str_cmp( name, "do_noemote" ))		return do_noemote;
        if ( !str_cmp( name, "do_negotiate" ))          return do_negotiate;
	if ( !str_cmp( name, "do_nooc"	))		return do_nooc;
...
*/	if ( !str_cmp( name, "do_oinvoke" ))		return do_oinvoke;
        if ( !str_cmp( name, "do_oldchanges" ))         return do_oldchanges;
...
/*	if ( !str_cmp( name, "do_opentourney" ))	return do_opentourney; */
	...
	if ( !str_cmp( name, "do_overthrow" ))		return do_overthrow;
        if ( !str_cmp( name, "do_overdrive" ))          return do_overdrive;
	if ( !str_cmp( name, "do_owhere" ))		return do_owhere;
	break;
    case 'p':
	if ( !str_cmp( name, "do_pager" ))		return do_pager;
        if ( !str_cmp( name, "do_palm" ))               return do_palm;
	...
	if ( !str_cmp( name, "do_polotics" ))		return do_polotics;
   ...
   if ( !str_cmp( name, "do_pkreg" ))    return do_pkreg;
	break;
    case 'q':
	...
	break;
    case 'r':
        if ( !str_cmp( name, "do_researchtarget" ))     return do_researchtarget;
	...
	if ( !str_cmp( name, "do_rebeep" ))		return do_rebeep;
	...
	if ( !str_cmp( name, "do_recall" ))              return do_recall;
        if ( !str_cmp( name, "do_rdesc"  ))             return do_rdesc;
	if ( !str_cmp( name, "do_recite" ))		return do_recite;
	if ( !str_cmp( name, "do_recite" ))             return do_recite;
        if ( !str_cmp( name, "do_redirect" ))           return do_redirect;
	...
	if ( !str_cmp( name, "do_remally" ))		return do_remally;
	if ( !str_cmp( name, "do_remoteopenbay" ))  	return do_remoteopenbay;
	...
	if ( !str_cmp( name, "do_repair" ))		return do_repair;
        if ( !str_cmp( name, "do_repairitem" ))         return do_repairitem;
	...
	if ( !str_cmp( name, "do_reset" ))		return do_reset;
        if ( !str_cmp( name, "do_reset_age" ))            return do_reset_age;
	...
	if ( !str_cmp( name, "do_stupid" ))		return do_stupid;
	...
	if ( !str_cmp( name, "do_setrank" ))		return do_setrank;
	if ( !str_cmp( name, "do_setwage" ))		return do_setwage;
        if ( !str_cmp( name, "do_setweather" ))         return do_setweather;
	if ( !str_cmp( name, "do_scrapattack" ))		return do_scrapattack;
	if ( !str_cmp( name, "do_search" ))		return do_search;
	if ( !str_cmp( name, "do_selfdestruct" ))	return do_selfdestruct;
	...
	if ( !str_cmp( name, "do_showallclans" ))	return do_showallclans;
	...
	if ( !str_cmp( name, "do_subclass" ))		return do_subclass;
	...
	if ( !str_cmp( name, "do_togglenh" ))		return do_togglenh;
	if ( !str_cmp( name, "do_toggleut" ))		return do_toggleut;
	if ( !str_cmp( name, "do_torture" ))		return do_torture;
	if ( !str_cmp( name, "do_trace_comlink" ))	return do_trace_comlink;
	...
	if ( !str_cmp( name, "do_trust" ))		return do_trust;
        if ( !str_cmp( name, "do_tuck" ))               return do_tuck;
	if ( !str_cmp( name, "do_typo" ))		return do_typo;
	break;
    case 'u':
        if ( !str_cmp( name, "do_unlinklaser"))         return do_unlinklaser;
	...
	if ( !str_cmp( name, "do_unhell" ))		return do_unhell;
        if ( !str_cmp( name, "do_unbind" ))             return do_unbind;
	...
	if ( !str_cmp( name, "do_wizdispel" ))		return do_wizdispel;
...
//    if ( spell == spell_faerie_fire )	    return "spell_faerie_fire";
    ...
    if ( spell == spell_frostball )	    return "spell_frostball";
    ...
    if ( spell == spell_research )	    return "spell_research";
    ...
    if ( spell == spell_darkmatter)	    return "spell_darkmatter";
    ...
    if ( spell == spell_disabled )       return "spell_disabled";
...
{   
    if ( skill == do_mp_refresh )		return "do_mp_refresh";
    if ( skill == do_selfdestruct )		return "do_selfdestruct";
    if ( skill == do_saveall )			return "do_saveall";
    if ( skill == do_stupid )			return "do_stupid";
    if ( skill == do_alias )			return "do_alias";
    if ( skill == do_antirestore )		return "do_antirestore";
    if ( skill == do_dbreath )       		return "do_dbreath";
    if ( skill == do_disabled )       		return "do_disabled";
    if ( skill == do_detach )			return "do_detach";
    if ( skill == do_abdicate )			return "do_abdicate";
    if ( skill == do_overthrow )		return "do_overthrow";
    if ( skill == do_arch )			return "do_arch";
    if ( skill == do_allpolotics )		return "do_allpolotics";
    if ( skill == do_decpeace )			return "do_decpeace";
    if ( skill == do_addally )			return "do_addally";
    if ( skill == do_award )			return "do_award";
    if ( skill == do_rembounty )		return "do_rembounty";
    if ( skill == do_remally )			return "do_remally";
    if ( skill == do_remove_module )		return "do_remove_module";
    if ( skill == do_i100 )			return "do_i100";
    if ( skill == do_i103 )			return "do_i103";
    if ( skill == do_i104 )			return "do_i104";
    if ( skill == do_i105 )			return "do_i105";
    if ( skill == do_i106 )                     return "do_i106";
    if ( skill == do_i107 )                     return "do_i107";
    if ( skill == do_teach )			return "do_teach";
    if ( skill == do_trace_comlink )		return "do_trace_comlink";
    if ( skill == do_gecko )			return "do_gecko";
    if ( skill == do_gather_intelligence )      return "do_gather_intelligence";
    if ( skill == do_add_patrol  )		return "do_add_patrol";
    if ( skill == do_elite_patrol )             return "do_elite_patrol";
    ...
    if ( skill == do_interdict )		return "do_interdict";
    ...
    if ( skill == do_subclass )			return "do_subclass";
    ...
    if ( skill == do_besiege )			return "do_besiege";
    ...
    if ( skill == do_availships )		return "do_availships";
    ...
    if ( skill == do_makeinoculator )           return "do_makeinoculator";
    ...
    if ( skill == do_togglenh )			return "do_togglenh";
    if ( skill == do_toggleut )			return "do_toggleut";
    ...
    if ( skill == do_remoteopenbay )			return 
"do_remoteopenbay";
    ...
    if ( skill == do_researchtarget )           return "do_researchtarget";
    ...
    if ( skill == do_cheap )			return "do_cheap";
    ...
    if ( skill == do_concentrate )              return "do_concentrate";
...
//    if ( skill == do_cutdoor )                  return "do_cutdoor";
    if ( skill == do_decwar )			return "do_decwar";
    if ( skill == do_delclan )			return "do_delclan";
    if ( skill == do_deleteship )			return 
"do_deleteship";
    ...
    if ( skill == do_docking )			return "do_docking";
...
//    if ( skill == do_immdemote )		return "do_immdemote";
//    if ( skill == do_immpromote )		return "do_immpromote";
    if ( skill == do_immortalize )		return "do_immortalize";
    if ( skill == do_immtalk )			return "do_immtalk";
    if ( skill == do_inoculate )                return "do_inoculate";
    if ( skill == do_iooc )			return "do_iooc";
    if ( skill == do_infochan )			return "do_infochan";
    ...
    if ( skill == do_loremedicine )             return "do_loremedicine";
    ...
    if ( skill == do_makebludgeon )		return "do_makebludgeon";
    if ( skill == do_makelightsaber )		return "do_makelightsaber";
    if ( skill == do_makedoublelightsaber )     return "do_makedoublelightsaber";
    if ( skill == do_makeforcepike )             return "do_makeforcepike";
    ...
    if ( skill == do_makeshot )                 return "do_makeshot";
    ...
    if ( skill == do_negotiate )		return "do_negotiate";
    if ( skill == do_noemote )			return "do_noemote";
    if ( skill == do_nooc	)		return "do_nooc";
...
*/    if ( skill == do_oinvoke )		return "do_oinvoke";
    if ( skill == do_oldchanges )               return "do_oldchanges";
...
/*  if ( skill == do_opentourney )		return "do_opentourney"; */
    ...
    if ( skill == do_palm )                     return "do_palm";
    ...
    if ( skill == do_polotics )			return "do_polotics";
    ...
    if ( skill == do_pkreg )       return "do_pkreg";
    ...
    if ( skill == do_repairitem )               return "do_repairitem";
    ...
    if ( skill == do_reset_age )                return "do_reset_age";
    ...
    if ( skill == do_scrapattack )			return "do_scrapattack";
    if ( skill == do_search )			return "do_search";
    if ( skill == do_setrank )			return "do_setrank";
    if ( skill == do_setwage )			return "do_setwage";
    ...
    if ( skill == do_showallclans )		return "do_showallclans";
    ...
    if ( skill == do_tuck )                     return "do_tuck";
    ...
    if ( skill == do_wizdispel )			return "do_wizdispel";
    ...
    if ( skill == do_overdrive )                 return "do_overdrive";
    return "reserved";
}
