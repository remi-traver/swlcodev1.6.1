...
char *  const   planet_flags [] =
{
  ...
  "af'el", "dosha", "Nar_Shadda", "bespin", "hosk", "Astroid Field",
  "Jaqua", "p20", "Xantic", "p22", "Ajina", "p24",
  "Yavin IV", "Zumonduland", "Neimoidia", "Wayland", 
  "Corellia", "p30", "p31"
};

char *  const   weapon_table    [WEAPON_MAX] =
{
    "none",
    "vibro-axe",  "vibro-blade",  "lightsaber", "whip", "slugthrower",
    "blaster",  "flamethrower", "bludgeon", "bowcaster", "sword",
    "force pike", "vibro-sword", "axe"
};

char *  const   spice_table    [] =
{
    "glitterstim", "carsanum", 
"ryll","andris","weed","opium","speed","shrooms","steroids","extacy"
};
...
"nomagic", "bank", "private", "safe", "donation","solitary",  "norecall",
"nodropall", "petshop", "silence", "logspeech", "nodrop", "clanstoreroom",
"plr_home", "empty_home", "teleport", "hotel", "nofloor", "refinery", "factory",
"republic_recruit", "empire_recruit", "spacecraft", "prototype", "imports"
};

char *	const	o_flags	[] =
{
"glow", "hum", "two_handed", "hutt_size", "contraband", "invis", "magic", 
"nodrop", "bless",
"antigood", "antievil", "antineutral", "noremove", "inventory",
"antisoldier", "antithief", "antihunter", "antijedi", "small_size", "large_size",
"demonsteel", "angelsteel", "relic", "antisith", "antipilot", 
...
"shield", "about", "waist", "wrist", "wield", "hold", "dual_wield", "ears", "eyes",
"_missile_", "trinket","r2","r3","r4","r5","r6",
...
"fightcomp", "midcomp", "capitalcomp","part","slugammo","inoculator", "scalpel",
"frigatecomp", "scapitalcomp", "medicine", "shot", "elixir", "carmor", "fuelcell"
};
...
"hyperspeed", "realspeed", "lasers", "shield", "energy", "missiles", 
"rockets", "torpedos", "tractorbeam", "comm", "sensor", "astro_array", 
"chaff", "manuever", "laserdamage", "salvo", "damageresistance", 
"magicresistance", "number_attacks", "attack_speed","vibro-blades",
"blasters","swords","lightsabers","unarmed","removeme", "negotiate", 
"sweapon", "removeme", "removeme", "ions", "engine", "generator", "hassembly",
"overdrive", "laserbattery", "sarmor", "cargo", "mlauncher", "tlauncher",
"rlauncher", "simulator", "whips", "bludgeons", "cloak", "axes", "flamethrowers",
"bowcasters", "slugthrowers", "forcepikes", "pickshiplock", "tuck"
 };

char *	const	a_flags [] =
{
"blind", "invisible", "detect_evil", "detect_invis", "detect_magic",
"detect_hidden", "weaken", "sanctuary", "masked_aura", "infrared", "tracecomlink",
"regeneration", "poison", "protect", "paralysis", "sneak", "hide", 
"sleep", "charm", "flying", "pass_door", "floating", "truesight", 
"detect_traps", "scrying", "dip_immunity", "silvertongue", "sit_aware", 
"iceshield", "possess", "berserk", "aqua_breath" };
...
"nosummon", "pageron", "notitled", "room", "bound", "disguised", 
...
"deny", "freeze", "killer","nooc", "litterbug", "ansi", "rip", "nice", 
...
{
"r1", "poison_gas", "poison_dart", "poison_needle", "poison_dagger", "poison_arrow",
"blindness_gas", "sleeping_gas", "flame", "explosion", "acid_spray", "electric_shock",
"blade", "sex_change", "r13", "r14", "r15", "r16", "r17", "r18", "r19", "r20",
"r21", "r22", "r23", "r24", "25", "r26", "r27", "r28", "r29", "r30", "r31", "r32"
};
/* By Arcturus */
char * const furniture_flags[] =
{
"siton", "sitin", "sitat", "standon", "standin", "standat", "sleepon", "sleepin",
"sleepat", "reston", "restin", "restat", "r12", "r13", "r14", "r15", "r16", "r17",
...
{
"closeable", "pickproof", "closed", "locked", "r4", "r5", "r6", "r7", "r8", "r9",
"r10", "r11", "r12", "r13", "r14", "r15", "r16", "r17", "r18", "r19", "r20", "r21", 
"r22", "r23", "r24", "r25", "r26", "r27", "r28", "r29","r30", "r31"
};

/* By Arcturus */
char * const medicine_types[MEDICINE_MAX] =
{
   "none", "hit", "move", "poison", "blindness", "cblind", "cpoison", "sleep"
};
...
/* Arcturus */
int get_traptype( char *type)
{
   int x;
   for (x = 0; x < 32; x++)
     if( !str_cmp(type, traptypes[x]) )
        return x;
   return -1;
}
/* Arcturus */
int get_cflag( char *flag )
{
    int x;
...
/* Arcturus */
int get_medtype( char *type )
{
    int x;

    for ( x = 0; x < MEDICINE_MAX; x++ )
      if ( !str_cmp( type, medicine_types[x] ) )
        return x;
    return -1;
}
 ...
 */
    ...
    victim->lastmset = ch->name; //for cheater tracking purposes ::grins evilly:: -- Raijen
	...
	{
	    send_to_char( "AC range is -1500 to 1500.\n\r", ch );
	    return;
	}
	victim->armor = value;
	if ( IS_NPC(victim) && IS_SET( victim->act, ACT_PROTOTYPE ) )
    ...
    {    
        if (IS_NPC(victim))
            send_to_char( "Mob's dont have ranks.\n\r", ch );
            return;

        if ( !can_mmodify( ch, victim ) )
            send_to_char( "Uhhh, no.\n\r", ch );
            return;

        if ( !str_cmp( arg3, "" ) )
        {
            send_to_char( "Put something.\n\r", ch );
            return;
        }

        smash_tilde( arg3 );
        DISPOSE( ch->pcdata->rank );
        ch->pcdata->rank = str_dup( "test" );
        send_to_char( "Ok.\n\r", ch );
        return;
    ...
    if ( !str_cmp( arg2, "bank" ) )
    {
        ...
        if ( value < 0 )
        {
          send_to_char( "How much was that?\n\r", ch );
          return;
        }

        victim->pcdata->bank = value;
        if (IS_NPC(victim))
          send_to_char( "Mobs dont use banks you idiot!\n\r", ch );
          return;
        return;
    }
	...
	{
	    send_to_char( "Value must be in range 1 to 100.\n\r", ch );
	    return;
	}
	victim->blood = value;
	return;
    ...
    if ( !str_cmp( arg2, "speaks" ) )
    {
	    ...
	    if ( !IS_NPC( victim ) )
	    {
    		int valid_langs = LANG_BASIC | LANG_SHRIIWOOK | LANG_TWI_LEK | LANG_RODIAN
    				| LANG_HUTTESE | LANG_MON_CALAMARI | LANG_NOGHRI | LANG_GAMORREAN
    				| LANG_JAWA | LANG_ADARIAN | LANG_EWOK | 
LANG_RADIO_WAVES | LANG_DEFEL | LANG_BINARY | LANG_BITH
...
*/
	...
	send_to_char( "  cflags key capacity condition\n\r",		ch );
	send_to_char( "For rawspice:            For ammo and batteries:\n\r",		ch );
	send_to_char( "  spicetype  grade #        charges (at least 1000 for ammo)\n\r",		ch );
	send_to_char( "For crystals:            For slugammo:\n\r",		ch );
	send_to_char( "  gemtype                  rounds caliber\n\r",		ch );
        send_to_char( "For Traps:\n\r", ch);
        send_to_char( "  traptype trapflag traplevel trapcharges\n\r", ch);
        send_to_char( "For furniture:\n\r", ch);
        send_to_char( "  fflags\n\r", ch);
        send_to_char( "For medicines:\n\r", ch);
        send_to_char( "  medtype1 medstrength1 medtype2 medstrength2\n\r", ch);
	return;
    }
...
*/
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "Possible Types:\n\r", ch );
           ...
           send_to_char( "Furniture    Trash      Container  Drinkcon    Landmine\n\r", ch );
           ...
           send_to_char( "Bolt         Chemical   Fightcomp  Midcomp     Capitalcomp\n\r", ch );	   
           send_to_char( "Piece        Slugammo   Inoculator Scalpel     Frigatecomp\n\r",ch);
           send_to_char( "Scapitalcomp Medicine   Shot       Elixir      Carmor\n\r", ch);
           send_to_char( "Fuelcell\n\r", ch);
	   return;
	}
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "small_size, human_size, large_size, hutt_size, contraband, two_handed\n\r", ch );
	   return;
	}
	while ( argument[0] != '\0' )
	{
	...
	if ( !argument || argument[0] == '\0' )
	{
	   send_to_char( "Usage: oset <object> wear <flag> [flag]...\n\r", ch );
           ...
           send_to_char( "wrist  wield    hold    ears    eyes   dual_wield\n\r", ch );
	   send_to_char( "trinket\n\r", ch);
	   return;
	}
	while ( argument[0] != '\0' )
	{
    ...
    if ( !str_cmp( arg2, "trapflag" ) )
    {
        if ( !can_omodify( ch, obj ) )
          return;
        if(obj->item_type != ITEM_TRAP)
        {
           send_to_char("This is not a trap.\n\r", ch);
           return;
        }
        if ( !argument || argument[0] == '\0' )
        {
           send_to_char( "Usage: oset <object> trapflag <flag> [flag]...\n\r", ch );
           send_to_char("room    obj    enter   leave    open    close\n\r", ch);
           send_to_char("get     put    pick    unlock   north   south\n\r", ch);
           send_to_char("east    west   up      down     examine\n\r", ch);
           return;
        }
        while ( argument[0] != '\0' )
        {
    ...
    if ( !str_cmp( arg2, "traptype" ) )
    {
        ...
        if ( !argument || argument[0] == '\0' )
        {
           send_to_char( "Usage: oset <object> traptype <type>...\n\r", ch );
           send_to_char("poison_gas    poison_dart   poison_needle   poison_dagger\n\r", ch);
           send_to_char("poison_arrow  blindness_gas sleeping_gas    flame\n\r", ch);
           send_to_char("explosion     acid_spray    electric_shock  blade\n\r", ch);
           send_to_char("sex_change\n\r", ch);
           return;
        }
        value = get_traptype(argument);
        if(value == -1)
        {
    ...
    if ( !str_cmp( arg2, "trapcharges" ) )
    {
        if ( !can_omodify( ch, obj ) )
          return;
        if(obj->item_type != ITEM_TRAP)
        {
           send_to_char("Thats not a trap.\n\r", ch);
           return;
        }
        obj->value[0] = value;
        if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
	...
	if ( !arg2 || arg2[0] == '\0' || !argument || argument[0] == 0 )
	{
	   send_to_char( "Usage: oset <object> affect <field> <value>\n\r", ch );
  	   ...
  	   send_to_char( "hyperspeed   realspeed    shields     lasers        energy\n\r", ch);	  
  	   send_to_char( "missiles     rockets      torpedos    tractorbeam   comm\n\r", ch );	 
	   send_to_char( "sensor       astroarray   chaff       manuever      salvo\n\r",ch);
           send_to_char( "ions         engine       generator   hassembly     overdrive\n\r", ch);
           send_to_char( "sarmor       cargo        mlauncher   tlauncher     rlauncher\n\r", ch);
           send_to_char( "simulator    whips        bludgeons   sweapon       laserdamage\n\r", ch);
           send_to_char( "laserbattery vibro-blades lightsabers swords        blasters \n\r", ch);
           send_to_char( "cloak        axes       flamethrowers bowcasters    forcepikes\n\r", ch);
           send_to_char( "pickshiplock palm         tuck\n\r", ch);
           return;
	}
	loc = get_atype( arg2 );
	if ( loc < 1 )
	...
	case ITEM_WEAPON:
		...
		if ( value < 0 )
		{
		    ...
		    send_to_char( "\n\rChoices:\n\r", ch );
		    send_to_char( "   none, vibro-axe, vibro-blade, lightsaber, whip, slugthrower, blaster, flamethrower, bludgeon, bowcaster, sword, force pike, vibro-sword, axe\n\r", ch);		    return;
		}
		tmp = 3;
		break;
	    }
	...
	case ITEM_AMMO:
        ...
        case ITEM_FUELCELL:
            ...
            break;
        case ITEM_SLUGAMMO:
            if( !str_cmp( arg2, "rounds" ) )           tmp = 0;
            if( !str_cmp( arg2, "round" ) )            tmp = 0;
            if( !str_cmp( arg2, "caliber" ))            tmp = 1;
            break;
	case ITEM_BATTERY:
	    if ( !str_cmp( arg2, "charges" ) )          tmp = 0;
	    if ( !str_cmp( arg2, "charge" ) )          tmp = 0;
	...
	case ITEM_SPICE:
		...
		if ( value < 0 )
		{
		    ...
		    send_to_char( "\n\rChoices:\n\r", ch );
		    send_to_char( "   extacy, steroids, speed, shrooms, weed, opium, glitterstim, carsanum, ryll, andris\n\r", ch );
		    return;
		}
		tmp = 0;
		break;
	...
	case ITEM_CONTAINER:
            if ( !str_cmp( arg2, "cflags" ) )
            {
               if ( !argument || argument[0] == '\0' )
               {
                  send_to_char( "Usage: oset <object> cflags <flag> [flag]...\n\r", ch );
                 send_to_char( "closeable pickproof closed locked\n\r", ch);
                 return;
               }
               while ( argument[0] != '\0' )
               {
                  argument = one_argument( argument, arg3 );
                  value = get_cflag( arg3 );
                  if ( value < 0 || value > 31 )
                     ch_printf( ch, "Unknown flag: %s\n\r", arg3 );
                  else
	...
	case ITEM_BUTTON:
	    ...
	    break;
        case ITEM_FURNITURE:
            if( !str_cmp( arg2, "fflags" ))
            {
               if ( !argument || argument[0] == '\0' )
               {
                  send_to_char( "Usage: oset <object> fflags <flag> [flag]...\n\r", ch );
                  send_to_char( "siton sitin standon standin standat sleepon sleepin\n\r", ch);
                  send_to_char( "sleepat reston restat\n\r", ch);
                  return;
               }
               while ( argument[0] != '\0' )
               {
                  argument = one_argument( argument, arg3 );
                  value = get_fflag( arg3 );
                  if ( value < 0 || value > 31 )
                    ch_printf( ch, "Unknown flag: %s\n\r", arg3 );
                  else
         ...
         case ITEM_SHOT:
         case ITEM_ELIXIR:
            if(!str_cmp(arg2, "medstrength1")) tmp = 1;
            if(!str_cmp(arg2, "medstrength2")) tmp = 3;
            if(!str_cmp(arg2, "medtype1") )
            {
               if(!argument || argument[0] == '\0' )
               {
                  send_to_char( "Usage: oset <object> medtype1 type...\n\r", ch );
                  send_to_char( "none hit move poison blindness cpoison cblind sleep\n\r", ch);
                  return;
               }
               argument = one_argument( argument, arg3 );
               value = get_medtype( arg3 );
               if( value < 0 || value > MEDICINE_MAX)
                  ch_printf(ch, "Unknown Type: %s\n\r", arg3);
               else
                  obj->value[0] = value;
               if ( IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) )
            ...
            if(!str_cmp(arg2, "medtype2") )
            {         
               if(!argument || argument[0] == '\0' )
               {
                  send_to_char( "Usage: oset <object> medtype2 type...\n\r", ch );
                  send_to_char( "none hit move poison blindness cpoison cblind sleep\n\r", ch);
                  return;
               }
               argument = one_argument( argument, arg3 );
...
/*
    if ( !can_rmodify( ch, location ) )
      send_to_char( "You mayproceed with redit.", ch );
*/
/*
    if ( !str_cmp( arg, "on" ) )
    {
...
*/
	...
	tarea = location->area;	if ( !tarea->first_reset )
	{
	...
	if ( !argument || argument[0] == '\0' )
	{
	   ...
	   send_to_char( "Usage: redit flags <flag> [flag]...\n\r", ch );
           ...
           send_to_char( "nofloor, prototype, refinery, factory, rebellion_recruit, empire_recruit\n\r", ch ); 
	   send_to_char( "spacecraft, auction, no_drive, can_land, can_fly, hotel, imports \n\r", ch );                                               
           return;
	}
	while ( argument[0] != '\0' )
	{
...
void edit_buffer( CHAR_DATA *ch, char *argument )
{
   ...
   if ( edit->size + strlen(argument) + 1 >= MAX_STRING_LENGTH - 1 )
	send_to_char( "The buffer is full.\n\r", ch );
   else
   {
        int b_end;
        int bm = 75;
...
/* Old stock foldarea...YICH! new one below - Parnic
...
/* New foldarea with 'all' support by Parnic */
void do_foldarea( CHAR_DATA *ch, char *argument )
{
    AREA_DATA	*tarea;
    char         arg[MAX_INPUT_LENGTH];
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Usage: foldarea <filename> [remproto] or foldarea all\n\r", ch );
	return;
    }
