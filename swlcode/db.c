...
sh_int   gsn_doublelightsaber_crafting;
sh_int   gsn_spice_refining;
sh_int   gsn_makeblade;
sh_int   gsn_makebludgeon;
...
sh_int   gsn_repairitem;
sh_int   gsn_makeforcepike;
...
sh_int			gsn_unarmed;
sh_int			gsn_swords;
sh_int			gsn_slugthrowers;
sh_int                  gsn_vibroaxes;
sh_int                  gsn_flamethrowers;
...
sh_int			gsn_trace_comlink;
sh_int                  gsn_researchtarget;
sh_int                  gsn_palm;
sh_int                  gsn_tuck;
...
sh_int                  gsn_scan;
sh_int			gsn_slice;
sh_int   			gsn_scrapattack;
sh_int                  gsn_deflect; /*Deflect by Arcturus */

/* Medical Skills */
sh_int                  gsn_makemedpac;
sh_int                  gsn_makeinoculator;
sh_int                  gsn_inoculate;
sh_int                  gsn_makeshot;
sh_int                  gsn_concentrate;
sh_int                  gsn_mix;
sh_int                  gsn_loremedicine;
...
void boot_db( bool fCopyOver )
{
	...
	lmonth		= lday   / 35;
	time_info.month	= lmonth % 10;
	time_info.year	= lmonth / 10;
...
//        ASSIGN_GSN( gsn_seduce , "seduce" );
        ...
        ASSIGN_GSN( gsn_besiege  , "besiege" );
...
// 	ASSIGN_GSN( gsn_stun,		"stun" );
	...
	ASSIGN_GSN( gsn_cure_crit,	"cure critical" );
	ASSIGN_GSN( gsn_heal,	"heal" );
	ASSIGN_GSN( gsn_fireball,	"fireball" );
	ASSIGN_GSN( gsn_frostball,	"frostball" );
...
//	ASSIGN_GSN( gsn_curse,		"curse" );
	...
	ASSIGN_GSN( gsn_ewok,           "ewok" );
        ...
        ASSIGN_GSN( gsn_makeinoculator,  "makeinoculator");
        ASSIGN_GSN( gsn_inoculate,       "inoculate");
        ASSIGN_GSN( gsn_makeshot,        "makeshot" );
        ASSIGN_GSN( gsn_concentrate,     "concentrate" );
        ASSIGN_GSN( gsn_mix,             "mix" );
        ASSIGN_GSN( gsn_cloak,           "cloak" );
        ASSIGN_GSN( gsn_loremedicine,    "loremedicine" );
	...
	for ( ; ; )
	{
	    ...
	    if ( strArea[0] == '$' )
		break;
	    log_string("Reading in area files... PRE L_A_F");

	    load_area_file( last_area, strArea );
	    
	}
	log_string("Reading in area files... DEBUG3");
	fclose( fpList );
    }
...
/*
 * Get an extra description from a list. Arcturus says not for objects.
 */
...
/* Arcturus says this ones better for objects */
char *get_extra_descr_obj( const char *name, EXTRA_DESCR_DATA *ed )
{
    for ( ; ed; ed = ed->next )
    {
        if ( is_name_prefix( name, ed->keyword ) )
            return ed->description;
    }
    return NULL;
}
...
void make_wizlist( )
{
  ...
  return; //added to convert to a fixed wizl
...
/*
 if (dentry->d_name[0] =='D' || 'E' || 'J' || 'A' || 'N' || 'O')
--> this is a bad idea right here, should dentry = readdir(dp); instead of 
-->  return;

 comented out by Shriak to fix the broken wizlist
because while ( dentry ) is already there, and you should not
close it out so soon, looks like when you guys took out my
name here you broke some shit hehe
}
  while ( dentry )
  {
...
*/
  ...
  buf[0] = '\0';
  unlink( WIZLIST_FILE );
  towizfile( "The Immortals of Star Wars Life" );
  towizfile( " ");
  ilevel = 65535;
  for ( wiz = first_wiz; wiz; wiz = wiz->next )
  {
   if ( wiz->level > LEVEL_AVATAR )
   {
	...
	buf[0] = '\0';
      ...
      switch(ilevel)
      {
	default:             towizfile( " Norse Gods of Metal" ); break;
	case MAX_LEVEL -  1: towizfile( " Coders/Big People"	);  break;
	case MAX_LEVEL -  2: towizfile( " Supervisors" );	   break;
	case MAX_LEVEL -  3: towizfile( " PR/Quest" );	   break;
	case MAX_LEVEL -  4: towizfile( " Builders" );	           break;
	case MAX_LEVEL -  5: towizfile( " Helpfiles/Annoyances" );	   break;
	case MAX_LEVEL -  0: towizfile( " Norse Gods of Metal" );	   break;
      }
    }
		 ...
		 char *loadst, char *notloadst )
{
	...
	if ( !shownl )
	   continue;
//	pager_printf(ch, "&z+----------------------------------------------------------------------------------+\n\r");
	pager_printf(ch, "&z|&c%-20s| &BRooms: &c%5d - %-5d"
		     " &BObjs: &c%5d - %-5d &BMobs: &c%5d - %-5d%s&z|\n\r",
    ...
    pager_printf( ch, "&z+----------------------------------------------------------------------------------+\n\r");
    pager_printf( ch, "&z|&BAreas listed: &c%-3d  &BLoaded: &c%-55d&z|\n\r", count, loaded );
    pager_printf( ch, "&z+----------------------------------------------------------------------------------+\n\r");
    return;
}
...
void do_check_vnums( CHAR_DATA *ch, char *argument )
{
    ...
    if (low_range < 1 || low_range > 104857600 )
    {
      send_to_char("Invalid argument for bottom of range.\n\r", ch);
      return;
    }

    if (high_range < 1 || high_range > 104857600 )
    {
