...
sh_int get_obj_resistance( OBJ_DATA *obj )
{
    ...
    if (obj->item_type == ITEM_ARMOR || obj->item_type == ITEM_WEAPON || obj->item_type == ITEM_CARMOR)
      resist += (obj->value[0]);
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, sh_int wear_bit )
{
    ...
    check_size = FALSE;
/*    if ( obj->item_type == ITEM_PIECE && ch->race != RACE_BORG )
    {
	act( AT_MAGIC, "Only droids may install parts.",
	    ch, NULL, NULL, TO_CHAR );
	act( AT_ACTION, "$n wants to use $p, but isn't a droid.",
	    ch, obj, NULL, TO_ROOM );
	return;
    }*/
   ...
   { 
	...
	case ITEM_WEAR_TRINKET:
	    if ( get_eq_char( ch, WEAR_TRINKET )
	    && !remove_obj( ch, WEAR_TRINKET, fReplace ) )
		return;
	    if ( !get_eq_char( ch, WEAR_TRINKET ) )
	    {
			if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
			{
				if ( !obj->action_desc || obj->action_desc[0]=='\0' )  
				{
					act( AT_ACTION, "$n wears $p as a trinket.", ch, obj, NULL, TO_ROOM);
					act( AT_ACTION, "You wear $p as a trinket.", ch, obj, NULL, TO_CHAR);
				} 
				else
				actiondesc( ch, obj, NULL );

			}	
			equip_char( ch, obj, WEAR_TRINKET );
			oprog_wear_trigger( ch, obj );
			return;
		}
...
*/
	...
	case ITEM_HOLD:
            if( (tmpobj = get_eq_char ( ch, WEAR_WIELD ) ) != NULL)
            {
               if(IS_OBJ_STAT( tmpobj, ITEM_TWO_HANDED) )
               {
                   send_to_char("Your current weapon is two handed, you have no free hands.\n\r", ch);
                   return;
               }
               if ( get_eq_char( ch, WEAR_DUAL_WIELD ) )
               {
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    {
        act( AT_PLAIN, "$p is a grenade, the gods dont wish to be blown up.", ch, obj, 0, TO_CHAR );
        return;
    }
...
/* omikron wanted this mudwide too, so i suppose we have another free room 
flag
...
/*          spell_identify (0, LEVEL_HERO - 1, ch, auction->item); */
    ...
    if (!str_cmp(arg1,"bid") )
    {
	    ...
	    /* to avoid slow auction, use a bigger amount than 100 if the bet
            ...
            */
            ...
            comparebet = auction->bet * 1.1;
  
            if (newbet < comparebet || newbet == auction->bet )
            {
                send_to_char ("You must at least bid 10 Percent over the current bid.\n\r",ch);
		sprintf(buf, "That would come to %d credits.\n\r",
                        comparebet);
		send_to_char(buf, ch);
                return;
            }
  ...
  }/*end that if way way up yonder ways*/
...
}/*end the whole frigin function whew!*/
...
void do_connect( CHAR_DATA *ch, char *argument )
{
 ...
 /* good connection  */
  { 
    if ( ( pObjIndex = get_obj_index( first_ob->value[2] ) ) == NULL )
    {
         send_to_char("That object doesn't seem to work properly, please inform the administration.\n\r", ch);
         return;
    }
    new_ob = create_object( get_obj_index( first_ob->value[2] ), ch->top_level );
    extract_obj( first_ob );
    extract_obj( second_ob );
    obj_to_char( new_ob , ch );
    act( AT_ACTION, "$n jiggles some pieces together...\r\n ...suddenly they snap in place, creating $p!", ch, 
new_ob,NULL,  TO_ROOM );
    act( AT_ACTION, "You jiggle the pieces together...\r\n ...suddenly they snap into place, creating $p!", ch, 
new_ob, NULL, TO_CHAR);
  ...
  else
  {
    ...
    act( AT_ACTION, "You try to fit them together every which way, but they just don't want to fit  together.", ch, 
NULL, NULL, TO_CHAR);
