 ...
 * Local functions.
 */
ch_ret crit_damage      args( ( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt ) );
...
ch_ret noparry_damage   args( ( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt ) );
...
//below from unarmed.c
ch_ret combo_hit(CHAR_DATA *ch, CHAR_DATA *victim);

bool check_hit (CHAR_DATA *ch, CHAR_DATA *victim, int prof_bonus)
{
    int victim_ac;
    int diceroll;
    int thac0;
...
ch_ret multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if ( who_fighting( ch ) != victim 
        || dt == gsn_backstab || dt == gsn_circle || dt == gsn_slice || dt == gsn_cheap)
	return rNONE;
	...
	learn_from_failure( ch, gsn_dual_wield );
      ...
      dual_bonus = 0;
    /* They do dual wield, with their fists :P */
    if(ch->subclass == SUBCLASS_MARTIST && get_eq_char(ch, WEAR_WIELD) == NULL)
    {
       dual_bonus = IS_NPC(ch) ? (ch->skill_level[COMBAT_ABILITY] / 10) : 10;
       retcode = one_hit( ch, victim, dt );
       if( retcode != rNONE || who_fighting(ch) != victim )
          return retcode;
       /* And one more attack for their speed bonus. */
       retcode = one_hit( ch, victim, dt );
       if( retcode != rNONE || who_fighting(ch) != victim )
          return retcode;
    }
	...
	/*
	 * Apply Bonus Attacks for PCs     - Raijen
	 */
    if ( ch->bonus_attacks > 0 )
    {
        for ( chance=0; chance <= ch->bonus_attacks; chance++ )
	{
  	   retcode = one_hit( ch, victim, dt );
  	   if ( retcode != rNONE || who_fighting( ch ) != victim )
  	   {
...
/* Sometimes mobs get an extra attack. */
    if((retcode != rNONE || who_fighting( ch ) != victim) && IS_NPC(ch) )
    {
...
int weapon_prof_bonus_check( CHAR_DATA *ch, OBJ_DATA *wield, int *gsn_ptr )
{
	...
	{  /* Some gsns used multiples times. */
           default:                     *gsn_ptr = -1;                  break;
           case WEAPON_NONE:            *gsn_ptr = gsn_unarmed;         break;
           case WEAPON_VIBRO_AXE:       *gsn_ptr = gsn_vibroaxes;       break;
           case WEAPON_VIBRO_BLADE:	*gsn_ptr = gsn_vibro_blades;	break;
           case WEAPON_LIGHTSABER:      *gsn_ptr = gsn_lightsabers;     break;
           case WEAPON_WHIP:	        *gsn_ptr = gsn_whip;	        break;
           case WEAPON_SLUGTHROWER:	*gsn_ptr = gsn_slugthrowers;	break;
           case WEAPON_BLASTER:   	*gsn_ptr = gsn_blasters;	break;
           case WEAPON_FLAMETHROWER:    *gsn_ptr = gsn_flamethrowers;   break;
           case WEAPON_BLUDGEON:	*gsn_ptr = gsn_bludgeons;	break;
           case WEAPON_BOWCASTER:	*gsn_ptr = gsn_bowcasters;	break;
           case WEAPON_SWORD:   	*gsn_ptr = gsn_swords;	        break;
           case WEAPON_FORCE_PIKE:	*gsn_ptr = gsn_force_pikes;	break;
           case WEAPON_VIBRO_SWORD:     *gsn_ptr = gsn_swords;          break;
           case WEAPON_AXE:             *gsn_ptr = gsn_vibroaxes;       break;
	}
    }
    if ( !IS_NPC(ch) && !wield )   
       *gsn_ptr = gsn_unarmed;
...
sh_int off_shld_lvl( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    {
	lvl = UMAX( 5, (ch->skill_level[ALTER_ABILITY])  );
...
ch_ret one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if ( (wield = get_eq_char( ch, WEAR_DUAL_WIELD )) != NULL 
         && dt != gsn_backstab && dt != gsn_circle && dt != gsn_slice )
    {
       if ( dual_flip == FALSE )
       {
	 dual_flip = TRUE;
    ...
    if( !wield )
      ...
      ch->move -= abs( wield->weight );
    if ( ch->move <= 0 )
      ch->move = 0;

    if ( !check_hit( ch, victim, prof_bonus) )
    {
	/* Miss. */
	if ( prof_gsn != -1 )
     ...
     * Hit.
     * Calc damage.
     * -- And calculate + apply Fatigue as well. -- Raijen
     *
     */
     ...
     */     
    dam += GET_DAMROLL(ch);
    /* What good is damplus for if it don't do shiT!
     * Well how about that, now it does.
     */
    if( IS_NPC(ch) )
      dam += ch->damplus;
    if ( prof_bonus )
    ...
    if ( !IS_AWAKE(victim) )
        dam *= 2;
    if ( dt == gsn_backstab )
        dam *= ((ch->skill_level[HUNTING_ABILITY] / 10));
    if ( dt == gsn_circle )
        dam *= ((ch->skill_level[HUNTING_ABILITY] / 20));    
    ...
    else if ( dt == (TYPE_HIT + WEAPON_FLAMETHROWER ) && wield && wield->item_type == ITEM_WEAPON )
    {
            ...
            )
     {
     	if ( wield->value[4] < 1  )
     	{ 
            act( AT_YELLOW, "$n waves a dead handle grip around in the air.",  ch, NULL, victim, TO_VICT    );
   ...
   {
    dam *= 2;
    if ( (retcode = crit_damage( ch, victim, dam, dt )) != rNONE )
      return retcode;
    if ( char_died(ch) )
      return rCHAR_DIED;
    if ( char_died(victim) )
...
ch_ret damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	if ( dt == (TYPE_HIT + 8) || dt == TYPE_HIT)
	  dam = ris_damage(victim, dam, RIS_BLUNT);
	else
	if ( dt == (TYPE_HIT + 2) || dt == (TYPE_HIT + 11) || dt == (TYPE_HIT + 5 ) )
	  dam = ris_damage(victim, dam, RIS_PIERCE);
	else
	if ( dt == (TYPE_HIT + 1)  || dt == (TYPE_HIT + 3)
	||   dt == (TYPE_HIT + 4)  || dt == (TYPE_HIT + 10)
	  ...
	  dam = ris_damage(victim, dam, RIS_SLASH);
        ...
        if( dt == (TYPE_HIT + 6) || dt == (TYPE_HIT + 9) )
          dam = ris_damage(victim, dam, RIS_ENERGY);
	if ( dam == -1 )
	{
	    if ( dt >= 0 && dt < top_sn )
  ...
  */
    ...
    if (dam > 10 && dt != TYPE_UNDEFINED && (IS_NPC(ch) && number_percent() >= 95))
    {
	/* get a random body eq part */
	dameq  = number_range(WEAR_LIGHT, WEAR_TRINKET);
...
ch_ret noparry_damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	if ( victim->position > POS_STUNNED && victim->position )
	{
	    if ( !victim->fighting )
		set_fighting( victim, ch );
	    if ( victim->fighting )
...
//    if ( !IS_NPC(ch) && dam >= (GET_DAMCAP(ch)))
//    {
//	   dam = (GET_DAMCAP(ch));
//    }
    if ( IS_NPC(ch) && dam >= ch->top_level * 5)
	   dam = ch->top_level * 5;
...
// thinking back, i really should have added a stop_fighting or somesuch, 
// but char_from_room should handle that.
// 
// - Raijen
	}	
...
ch_ret crit_damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    char buf[MAX_STRING_LENGTH];
    char buf1[MAX_STRING_LENGTH];
    sh_int dameq;
    ...
    if(!IS_NPC(ch))
    {
      ...
      && (dt == (TYPE_HIT + WEAPON_SWORD ) || dt == (TYPE_HIT + WEAPON_LIGHTSABER) 
      || dt == (TYPE_HIT + WEAPON_VIBRO_SWORD))  )
          dam *= 1.5;
    }
    if ( victim->position == POS_DEAD )
	return rVICT_DIED;
...
//    if ( dampmod > 1000000 )
  ...
  //    dam = ( dam * dampmod ) / 1000000;


    if ( IS_NPC(ch) && dam >= ch->top_level * 5)
	   dam = ch->top_level * 10;
    send_to_char("&RYou score a critical hit.&w\n\r", ch);
    act( AT_RED, "$n gets a critical hit.", ch, victim, NULL, TO_ROOM );
    if ( GET_DAMAGE_RESISTANCE(victim) > 0)
    {
	dam = ( dam * ( 100 - UMIN(GET_DAMAGE_RESISTANCE(victim), 80 ) ) / 100 );
    }
     ...
     * bonuses/penalties for having or not having equipment where hit
     */
    if (dam > 10 && dt != TYPE_UNDEFINED && (IS_NPC(ch) && number_percent() >= 50))
    {
	/* get a random body eq part */
	dameq  = number_range(WEAR_LIGHT, WEAR_TRINKET);
	damobj = get_eq_char(victim, dameq);
...
/* swreality chnages begin here */
  ...
  rename( buf, buf2 );
  remroster(victim);
  /* Comment Above rename, and uncomment copy, for infinite clones. 
  copy(buf, buf2);
  */
  return;
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	{
//	   sprintf( buf, "Thou hast done well by defeating %s.  Thy experience increases by %d.\n\r",victim->name, xpg);	   
	   ...
	   send_to_char( buf, gch );
        ...
        {
           xp = URANGE( members, xp*members*2, (exp_level( gch->skill_level[LEADERSHIP_ABILITY]+1) - exp_level(gch->skill_level[LEADERSHIP_ABILITY] )/10) );  
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim )
{
    ...
    /* Anti-2HourCombatAv Code :) Modified slightly by Arcturus to not 
       be as restraining, and to allow for 150 combats. original was 
       level 51, and gch->skill_level[COMBAT_ABILITY] -50 */

    if ( gch->skill_level[COMBAT_ABILITY] >= 75 )
	xp = xp / ( ( ( gch->skill_level[COMBAT_ABILITY] - 25 ) / 2 ) + 1 );    
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
   ...
   /*		     10 * percent					*/
    ...
    else if ( dampc <=   50 ) { vs = "damage";	vp = "damages";		}
    ...
    else if ( dampc <=  250 ) { vs = "maim";	vp = "maims";		}
    else if ( dampc <=  500 ) { vs = "MUTILATE";vp = "MUTILATES";	}
    else if ( dampc <=  750 ) { vs = "DISEMBOWEL";vp = "DISEMBOWELS";	}
    else if ( dampc <=  1000 ) { vs = "MASSACRE";  vp = "MASSACRES";	}
    else if ( dampc <=  1500 ) { vs = "PULVERIZE"; vp = "PULVERIZES";	}
    else if ( dampc <=  2000 ) { vs = "EVISCERATE";vp = "EVISCERATES";	}
    else if ( dampc <=  3000 ) { vs = "* OBLITERATE *";
			        vp = "* OBLITERATES *";			}
    else                      { vs = "*** ANNIHILATE ***";
			        vp = "*** ANNIHILATES ***";		}
    ...
    {    
         ...
         sprintf( sound , "!!SOUND(blaster V=%d) \n\r" , vol );
	...
	sprintf( buf2, "Your hit %s $N &W[&R%d&W]%c",   vp, dam, punct );
	...
	sprintf( buf1, "$n's poisoned %s %s $N &W[&R%d&W]%c", attack, vp, dam, punct );
	sprintf( buf2, "Your poisoned %s %s $N &W[&R%d&W]%c", attack, vp, dam, punct );
	sprintf( buf3, "$n's poisoned %s %s you &W[&R%d&W]%c", attack, vp, dam, punct ); 
    }
    else
    {
	if ( skill )
	...
	sprintf( buf2, "Your %s %s $N &W[&R%d&W]%c",  attack, vp, dam, punct );
...
void do_murder( CHAR_DATA *ch, char *argument )
{
    ...
    if(IS_NPC(ch))
    {
        send_to_char("Your a mob, use kill.\n\r", ch);
        return;
    }
    if ( arg[0] == '\0' )
    {
    ...
    if ( victim == ch )
    {
	send_to_char( "If you really want to do that, use suicide.\n\r", ch );
	return;
    }
...
void do_slay( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( victim->name, "arcturus" ) ||  !str_cmp( victim->name, "soul" )  || !str_cmp( victim->name, "shiro" ) )
    {
        send_to_char( "You failed.\n\r", ch );
        act( AT_LBLUE, "$n has tried to kill you.", ch, NULL, victim, TO_VICT    );
        return;
    }

    if ( ch == victim )
    {
	send_to_char( "Use suicide to delete your character.\n\r", ch );
        return;
    }
    ...
    else if ( !str_cmp( arg2, "armageddon" ) )
    {
      act( AT_WHITE, "&c&CYou release the wrath of the heavens upon $N", ch, NULL, victim, TO_CHAR );
      act( AT_WHITE, "&C&c$n strikes you down with a fiery bolt of fury!", ch, NULL, victim, TO_VICT );
      act( AT_WHITE, "&zWith a fiery blast from the heavens, $n releases a volley of brimstone at $N!", ch, NULL, victim, TO_NOTVICT );
    }
    ...
    else if ( !str_cmp( arg2, "smite" ) )
    {
      act( AT_LBLUE, "You reach out and smite $N with a holy bolt from your hand!",  ch, NULL, victim, TO_CHAR    );
      act( AT_LBLUE, "$n reaches out and smites you with a wave of his hand!", ch, NULL, victim, TO_VICT    );
      act( AT_LBLUE, "$n stretches his arm and smites $N with a wave of his hand!",  ch, NULL, victim, TO_NOTVICT );
    }

    else if ( !str_cmp( arg2, "decap" ) )
    {
      act( AT_IMMORT, "&R&rA Guillotine comes down from the sky and &RSLICES&r $N's Head off!", ch, NULL, victim, TO_CHAR );
      act( AT_IMMORT, "Out of nowhere a Shining Metal Blade comes down and &RSLICES&W your head off!", ch, NULL, victim, TO_VICT );
      act( AT_IMMORT, "A Blade comes down from the sky and &RChops&W Off &N's Head!", ch, NULL, victim, TO_NOTVICT );
    }

    else if ( !str_cmp( arg2, "shotokan" ) )
    {
      act( AT_IMMORT, "&Ryou casually extend your hand and tear $N's throat out. They didn't see that one coming!", ch, NULL, victim, TO_CHAR );
      act( AT_IMMORT, "&zThe cold wind blows amongst the icy reeds. You have been touched by death.", ch, NULL, victim, TO_VICT );
      act( AT_IMMORT, "&rWith a clawed grasp, $n has struck $N with the touch of death. There could only be but one outcome.", ch, NULL, victim, TO_NOTVICT );
    }
