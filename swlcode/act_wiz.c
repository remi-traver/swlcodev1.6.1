...
void do_arch( CHAR_DATA *ch, char *argument )
{
  ch_printf( ch, "Function not yet complete\n\r");
}
...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
	...
	for ( cmd = command_hash[hash]; cmd; cmd = cmd->next )
	    ...
	    {
	pager_printf( ch, "&Y[&G%d&Y]&B %-15s", cmd->level, cmd->name );
		if ( ++col % 4 == 0 )
		    send_to_pager( "\n\r", ch );
	    }
...
/* Another  project done and over with */
...
void do_authorize( CHAR_DATA *ch, char *argument )
{
  ...
  if (!IS_IMMORTAL(ch) && ch->newbiehelper == 0)
  {
    send_to_char( "Sorry, only imms and newbiehelpers can auth at present\n\r", ch);
    return;
  }
  ...
  if ( arg2[0]=='\0' || !str_cmp( arg2,"accept" ) || !str_cmp( arg2,"yes" ))
	...
	ch_printf( ch, "You have authorized %s.\n\r", victim->name);
     ...
     /* Below sends a message to player when name is accepted - Brittany   */                           
        ...
        "You are now fully authorized to play Star Wars Life.\n\r",victim->name);                               /* B */
     return;
     }
...
/* GAY Removed by Arcturus
  if ( ch->pcdata->status >= 1 && !IS_IMMORTAL(ch))
  {
    send_to_char( "Players with honour may not change their ranks\n\r", ch );
...
void do_ostat( CHAR_DATA *ch, char *argument )
{
	...
	ch_printf( ch, "Action description: %s&w.\n\r", obj->action_desc );
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
	...
	get_curr_lck(victim),
	get_curr_frc(victim) );
    ch_printf( ch, "Hps: %d/%d  Force: %d/%d   Move: %d/%d   Blood: %d\n\r",
...
ch_printf( ch,
    ...
    ch_printf( ch, "Years: %d   Seconds Played: %d   Timer: %d   Act: %d   SC:%d\n\r",
	get_age( victim ), (int) victim->played, victim->timer, victim->act, victim->subclass );
    if ( IS_NPC( victim ) )
    {	
	...
	{
	ch_printf( ch, "Cheaters Never Prosper!  Mset-%s  Advance-%s\n\r",
	  victim->lastmset, victim->lastadvance );
	}
...
void do_balzhur( CHAR_DATA *ch, char *argument )
{
  ...
  if ( IS_NPC(ch) )
   ...
   {
	send_to_char("Balzhur laughs at you, as you are not high enough on the\n",ch);
	send_to_char("food chain to send them to be eaten by Balzhur!\n\n",ch);
	return;
   }
    ...
    {
      ch_printf( ch, "Unknown error #%d - %s (immortal data).  Report to Arcturus\n\r", errno, strerror( errno ) );
    ...
    for ( pArea = first_build; pArea; pArea = pArea->next )
      if ( !strcmp( pArea->filename, buf2 ) )
      {
        ...
        {
          ch_printf( ch, "Unknown error #%d - %s (area data).  Report to Arcturus.\n\r",
...
void do_advance( CHAR_DATA *ch, char *argument )
{
  ...
  if ( IS_NPC(ch) )
     ...
     *   -- Swiftest
     */
    victim->lastadvance = ch->name; //for cheater tracking purposes
	...
	send_to_char( "Cursed and forsaken! The gods have lowered your level.\n\r", victim );
        victim->experience[ability] = exp_level(level);
        for( iLevel = victim->skill_level[ability]; iLevel > level; iLevel-- )
        {
           deadvance_level( victim, ability );
        }    
    }
    else
    {
...
/*added by Shriak, making imm's easier to manage*/
/*also forces victim to save and clone*/
/*then commented out by Raijen, as max-levels are calculated by update.c anyhow.*/
...
if ( arg == '\0')
 {
  send_to_char( "setmax: SYNTAX setmax <charector> <ability>", ch );
  return;
 }
...
if ( ch->MAX_ABILITY[ability])
{    
return ch->MAX_ABILITY[ability];
}
**********/
}

/* removed in favor of new immortalize - Parnic
void do_immdemote(CHAR_DATA *ch, char *argument)
{
...
argument = one_argument( argument, arg );
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Syntax: immdemote  <char>\n\r", ch );
	return;
    }
...
{
	send_to_char("You stop yourself when you realize that they are still mortal.\r\n",ch);
	send_to_char("You take pity on them, and decide to leave them alone...\r\n",ch);
	return;
}

    send_to_char( "Immortalizing: demoting player...\n\r", ch );
    set_char_color( AT_IMMORT, victim );
    act( AT_IMMORT, "$n Black electricity surges from him... then raises $s arms to the 
sky...",ch, NULL, NULL, TO_ROOM );
    set_char_color( AT_WHITE, victim );
    send_to_char( "You suddenly feel very weak...\n\r\n\r", victim );
    set_char_color( AT_LBLUE, victim );

    send_to_char( "You awake....\n\r", victim );

COMMENT:  cant demote at or above your level

if (victim->top_level >= ch->top_level)
{
	act( AT_IMMORT, "$n collapses. He is not powerful enough...",ch,NULL,NULL,TO_ROOM);
	send_to_char("You collapse, and realize that you aren't a powerful enough\n\r",ch);
	send_to_char("God to take their power.\n\r",ch);
	return;
}

if (victim->top_level >= LEVEL_SUPREME)
{
	send_to_char("You cant do that to a SUPREME GOD!\r\n",ch);
	return;
}

victim->top_level--;

if (victim->top_level == LEVEL_AVATAR) COMMENT: demoted to level 100
{
	send_to_char("You feel weaker as a HIGHER GOD has taken away some of your\n\r",victim);
	send_to_char("power.  You loose your immortality.  You have become an AVATAR!\n\r",victim);
	do_save(victim,"");
	do_clone(victim,"");
	return;
}
if (victim->top_level == LEVEL_NEOPHYTE) COMMENT: demoted to level 101
{
	send_to_char("You feel weaker as a HIGHER GOD has taken away some of your\n\r",victim);
	send_to_char("power.  You are no longer an ARCHANGEL!\n\r",victim);
	send_to_char("  You have become IMMORTAL!\n\r",victim);
	do_save(victim,"");
	do_clone(victim,"");

	return;
}
if (victim->top_level == LEVEL_CREATOR) COMMENT: demoted to level 102
{
	send_to_char("You feel weaker as a HIGHER GOD has taken away some of your\n\r",victim);
	send_to_char("power.  Your godly powers hae been taken away.\n\r",victim);
	send_to_char("You have become an ARCHANGEL!\n\r",victim);
	do_save(victim,"");
	do_clone(victim,"");
	return;
}
if (victim->top_level == LEVEL_GOD)  COMMENT: demoted to level 103
{
	send_to_char("You feel weaker as a HIGHER GOD has taken away some of your\n\r",victim);
	send_to_char("power.  Some of your godly powers hae been taken away.\n\r",victim);
	send_to_char("You have become a LESSER GOD!\n\r",ch);
	do_save(victim,"");
	do_clone(victim,"");
	return;
}

COMMENT: 105's cant be demoted to level 104!
...
/* removed by Parnic - new immortalize does the same thing
void do_immpromote(CHAR_DATA *ch, char *argument)
{
...
argument = one_argument( argument, arg );
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Syntax: immpromote  <char>\n\r", ch );
	return;
    }
    ...
    send_to_char( "Immortalizing: promoting player...\n\r", ch );
    set_char_color( AT_IMMORT, victim );
...
COMMENT: cant promote to or higher than your level or to 105

if ((victim->top_level >= ch->top_level)||(victim->top_level == 104))
{
	act( AT_IMMORT, "$n collapses. He is not powerful enough...",ch,NULL,NULL,TO_ROOM);
	send_to_char("You collapse, and realize that you aren't a powerful enough\n\r",ch);
	send_to_char("God to give them that much power.\n\r",ch);
	return;
}

COMMENT: if it's just a normalplayer or an avatar, set level to avatar, otherwize 
promote then promote

if (victim->top_level < LEVEL_AVATAR)
{
	victim->top_level = LEVEL_AVATAR;
	send_to_char("A mystical shroud surrounds you, as you transform\n\r",victim);
	send_to_char("an AVATAR!  Token of the Gods!\n\r",victim);
	do_save(victim,"");
	do_clone(victim,"");
	return;
}
...
if (victim->top_level == LEVEL_NEOPHYTE)  COMMENT: was promoted to 101
{
	send_to_char("You raise your arms high, and feel your new power surging\n\r",victim);
	send_to_char("through your veins!  You have become IMMORTAL!\n\r",victim);
	do_save(victim,"");
	do_clone(victim,"");
	return;
}
if (victim->top_level == LEVEL_CREATOR)  COMMENT: was promoted to 102
{
	send_to_char("You raise your arms high, and feel your new power surging\n\r",victim);
	send_to_char("through your veins!  You have become an ARCHANGEL!\n\r",victim);
	do_save(victim,"");
	do_clone(victim,"");
	return;
}
if (victim->top_level == LEVEL_GOD) COMMENT: was promoted to 103
{
	send_to_char("You raise your arms high, and feel your new power surging\n\r",victim);
	send_to_char("through your veins!  You have become a LESSER GOD!\n\r",victim);
	do_save(victim,"");
	do_clone(victim,"");
	return;
}
if (victim->top_level == LEVEL_ETERNAL) COMMENT: was promoted to 104
{
	send_to_char("You raise your arms high, and feel your new power surging\n\r",victim);
	send_to_char("through your veins!  You have become A GREATER GOD!\n\r",victim);
	do_save(victim,"");
	do_clone(victim,"");
	return;
}
...
/* Old immortalize....new below - Parnic
void do_immortalize( CHAR_DATA *ch, char *argument )
{
    ...
    COMMENT THIS OUT: advance_level( victim );
...
void do_immortalize( CHAR_DATA *ch, char *argument )
{
    ...
    if(arg[0] == '\0')
    {
        send_to_char("You need to pick a person to immortalize.\r\n",ch);
        return;
    }
    if ( ( victim = get_char_room( ch, arg ) ) == NULL )
    {
    ...
    if(arg2[0] == '\0')
    {
        send_to_char("Syntax: Immortalize <charname> <immlevel>",ch);
        return;
    }
	level = atoi(arg2);
    if(level < 101 || level > 107 || level > get_trust(ch))
    {
	...
	ch_printf( ch, "\n\rYou advanced %s from level %d to %d\n\r" , victim->name , oldlevel , level );
	ch_printf( victim, "\n\r%s advanced you from level %d to %d\n\r" , ch->name , oldlevel , level );
	return;
}
...
void do_antirestore( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Anti-Restore whom?\n\r", ch );
	return;
    }
   	...
   	for ( vch = first_char; vch; vch = vch_next )
	{
		...
		vch->move = 0;
		update_pos (vch);
  sprintf( buf, "&R%s has &YA&Rn&YT&Ri&YR&Re&YS&Rt&YO&Rr&YE&Rd %s!!!"
           , ch->name, vch->name );
//  echo_to_all( AT_IMMORT, buf, vch ); 
      echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
    ...
    update_pos( victim );
    if ( ch != victim )
//  act( AT_IMMORT, "$n has AnTiReStOrEd you.", ch, NULL, victim, TO_VICT );
  sprintf( buf, "&R%s has &YA&Rn&YT&Ri&YR&Re&YS&Rt&YO&Rr&YE&Rd %s!!!"
           , ch->name, victim->name );
//  echo_to_char( AT_IMMORT, buf, victim ); 
      echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
...
void do_freeze( CHAR_DATA *ch, char *argument )
{
	...
	sprintf( buf, "&c&C%s has been unfrozen!", victim->name );
	echo_to_all( AT_WHITE, buf, 0 );
	//send_to_char( "FREEZE removed.\n\r", ch );
    }
    else
    {
	SET_BIT(victim->act, PLR_FREEZE);
	send_to_char( "You can't do SHIT!\n\r", victim );
	sprintf( buf, "&c&COooooo, %s is frozen like eyce!!!", victim->name );
	echo_to_all( AT_WHITE, buf, 0 );
...
void do_litterbug( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "LITTERBUG removed.\n\r", ch );
      sprintf( buf, "%s is no longer considered a LITTERBUG!!!"
         , victim->name );
      echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
    }
    else
...
void do_nooc( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    ...
    if ( !IS_IMMORTAL(ch) && ch->newbiehelper != 1 )
    {
	send_to_char( "This command is restricted to immortals and newbiehelpers", ch);
	return;
    }

    if ( arg[0] == '\0' )
    {
	send_to_char( "No ooc whom?\n\r", ch);
        return;
    }
    ...
    if( ch == victim )
    {
        send_to_char("On yourself? I don't think so.\n\r", ch);
        return;
    }

    if ( IS_IMMORTAL(victim) && get_trust(ch) < get_trust(victim) )
    {
	send_to_char( "Not on someone higher up, sorry.", ch);
	return;
    }        

    if ( IS_SET(victim->act, PLR_NOOC) )
    {
	REMOVE_BIT(victim->act, PLR_NOOC);
        send_to_char( "You can use the ooc channel now!\n\r", victim );
        sprintf( buf, "%s can use ooc again! Hurrah!", victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL);
    }
    else
    {
        SET_BIT(victim->act, PLR_NOOC);
        send_to_char( "Your ooc privilages have been revoked.\n\r", victim);
        sprintf( buf, "%s has had their ooc privilages taken away!", victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL);
    }
...
void do_noemote( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "NO_EMOTE removed.\n\r", ch );
        sprintf( buf, "By the grace of the gods, %s is no longer NOEMOTED!!!"
           , victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
	...
	send_to_char( "NO_EMOTE set.\n\r", ch );
        sprintf( buf, "%s has been NOEMOTED!!!"
           , victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
...
void do_notell( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "NO_TELL removed.\n\r", ch );
        sprintf( buf, "By the grace of the gods, %s is no longer NOTELLED!!!"
           , victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
	...
	send_to_char( "NO_TELL set.\n\r", ch );
      sprintf( buf, "%s has been NOTELLED!!!"
           , victim->name );
      echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
...
void do_notitle( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET(victim->pcdata->flags, PCFLAG_NOTITLE) )
    {
        ...
        sprintf( buf, "By the grace of the gods, %s is no longer NOTITLED!!!"
           , victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
    ...
    else
    {
        ...
        if(IS_SET(victim->pcdata->flags, PCFLAG_DISGUISED))
        {
           REMOVE_BIT(victim->pcdata->flags, PCFLAG_DISGUISED);
           send_to_char( "Your disguise fizzles.\n\r", victim);
           STRFREE(victim->pcdata->disguisename);
        }
        send_to_char( "You can't set your own title!\n\r", victim );
        send_to_char( "NOTITLE set.\n\r", ch );
        sprintf( buf, "%s has been NOTITLED!!!"
           , victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
...
void do_silence( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "SILENCE set.\n\r", ch );
      sprintf( buf, "%s has been SILENCED!!!"
           , victim->name );
      echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
    }
...
void do_unsilence( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "You can use channels again.\n\r", victim );
 	send_to_char( "SILENCE removed.\n\r", ch );
      sprintf( buf, "%s has been UNSILENCED!!!"
           , victim->name );
      echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
...
void do_users( CHAR_DATA *ch, char *argument )
{
   ...
   send_to_pager(buf, ch);
	...
	&& ( !str_cmp( d->character->name, "Raijen") )
        )
	{
	    count++;
	    pager_printf( ch,
...
void do_force( CHAR_DATA *ch, char *argument )
{
	...
	if (vch->name == "Shriak")
	{
		send_to_char( "Screw you buddy!\n\r", ch );
		return;
	}


	    if ( !IS_NPC(vch) && get_trust( vch ) < get_trust( ch ) && !IS_SET(vch->act, PLR_AFK ) )
	    {
	...
	if ( ( victim = get_char_world( ch, arg ) ) == NULL )
	{
        ...
        {
            send_to_char( "That player is AFK, please do not force him to do anything...\n\r", ch );
            return;
        }

	if (victim->name == "Shriak")
	{
		send_to_char( "Screw you buddy!\n\r", ch );
		return;
	}
...
void do_destroy( CHAR_DATA *ch, char *argument )
{
  ...
  if (get_trust(victim) > 106 && get_trust(ch) < 107 && get_trust(victim) < get_trust(ch))
  {
     send_to_char( "Unable To Comply -- You cannot kill God yet.\n\r", ch );
     return;
  }
  ...
  if ( !rename( buf, buf2 ) )
  {
    ...
    send_to_char( "Player destroyed.  Pfile saved in backup directory.\n\r", ch );
    	sprintf( buf, "You hear %s scream in the distance as one of the Elder Gods eradicates them!!!", victim->name );
	remroster(victim);
	echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); sprintf( buf, "%s%s", GOD_DIR, capitalize(arg) );
  sprintf( buf, "%s%c/%s.clone", PLAYER_DIR, tolower(arg[0]),
          capitalize( arg ) );
  sprintf( buf2, "%s%c/%s.clone", BACKUP_DIR, tolower(arg[0]),
          capitalize( arg ) );
	rename(buf, buf2);
...
void do_hell( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf( buf, "%s has been helled for %d %s%s!!!"
           , victim->name , time
           , (h_d ? "hour" : "day"), (time == 1 ? "" : "s") );
  echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
...
void do_unhell( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf( buf, "%s has been UNHELLED!!!"
           , victim->name );
  echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
...
void do_wizdispel( CHAR_DATA *ch, char *argument )
{
/*    char arg1[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    int cnt;
    ...
    if ( arg1[0] == '\0' )
    {
	send_to_char( "Syntax: wizdispel <char>.\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "Unable to Wizdispel, Reason = No affects!\n\r", ch );
	return;
    }

    send_to_char( "Dispeling player!\n\r", ch );
    set_char_color( AT_IMMORT, victim );
    send_to_char( "A god has removed all affects on you!\n\r", victim );
...
void do_toggleut( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  ...
  if ( victim->canultimate == 0 )
  {
	victim->canultimate = 1;
	ch_printf(victim, "%s has awarded you the UltimateTalk Channel.\n\rSyntax is 'ultimate <MESSAGE>'.\n\r", ch->name);
	save_char_obj(victim);	/* used to save ch, fixed by Thoric 09/17/96 */
  }
  else	
  {
	victim->canultimate = 0;
	ch_printf(victim, "%s has revoked your UltimateTalk Privilages\n\r", ch->name);
	save_char_obj(victim);	/* used to save ch, fixed by Thoric 09/17/96 */
  }
  return;
}
...
void do_forsake( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !*arg )
  {
    send_to_char( "Who has earned your ire?\n\r", ch );
    return;
  }
  if ( !(victim = get_char_world(ch, arg)) || IS_NPC(victim) )
  {
  ...
  {
	victim->forsaken = 1;
	ch_printf(victim, "You have earned the ire of the gods and have been forsaken by %s.\n\r", ch->name);
        sprintf( buf, "&Y%s&G has earned the ire of the gods and has been &rF&Ro&rr&Rs&ra&Rk&re&Rn&G", victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL);
	save_char_obj(victim);	/* used to save ch, fixed by Thoric 09/17/96 */
  }
  else	
  {
	victim->forsaken = 0;
	ch_printf(victim, "%s has deemed you to have redeemed yourself\n\r", ch->name);
        sprintf( buf, "%s has redeemed themself and is no longer forsaken", victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL);
...
void do_bless( CHAR_DATA *ch, char *argument )
{
  ...
  if(!( !strcmp(ch->name , "Shiro") || !strcmp(ch->name , "Arcturus") ))
  {
    send_to_char( "Only The Certain Immortals May Bless People!\n\r", ch);
    return;
  }
  ...
  {
	victim->blessed = 1;
	ch_printf(victim, "You have been blessed by the immortal %s.\n\r", ch->name);
        sprintf( buf, "&Y%s&B has earned the pleasure of the gods and is &CB&wL&CE&wS&CS&wE&CD&B!", victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL);
	save_char_obj(victim);	/* used to save ch, fixed by Thoric 09/17/96 */
  }
  else	
  {
	victim->blessed = 0;
	ch_printf(victim, "%s smiles, as the blessing expires.\n\r", ch->name);
...
void do_togglenh( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  ...
  if ( victim->newbiehelper == 0 )
  {
	victim->newbiehelper = 1;
	ch_printf(victim, "%s has awarded you Newbiehelper privelages.  See Shiro for Details.\n\r", ch->name);
	save_char_obj(victim);	/* used to save ch, fixed by Thoric 09/17/96 */
  }
  else	
  {
	victim->newbiehelper = 0;
	ch_printf(victim, "%s has revoked your Newbihelper Privilages\n\r", ch->name);
	save_char_obj(victim);	/* used to save ch, fixed by Thoric 09/17/96 */
  }
  return;
}
...
void do_smite( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Who gets the big Smite?\n\r", ch );
	return;
    }
    ...
    if ( IS_IMMORTAL(victim) )
    {
	send_to_char( "Not on a god, sorry.\n\r", ch);
	return;
    }
	

	set_char_color( AT_WHITE, ch );
	send_to_char( "You press the smite button and giggle!\n\r", ch );
	set_char_color( AT_IMMORT, victim );
	sprintf( buf, "%s has reached down from the heavens and has smitten %s!!!'", ch->name, victim->name );
	echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 

	victim->max_hit  = victim->max_hit * 0.9;
	if ( victim->max_mana > 0)
	  victim->max_mana = victim->max_mana * 0.9;
	victim->max_move = victim->max_move * 0.9;
    victim->hit  = 0;
    victim->mana = 0;
    victim->move = 0;
	return;
...
void do_iooc( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    sh_int color;
    int target;
    ...
    if ( !IS_IMMORTAL(ch) )
    {
        send_to_char( "Only imms may IOoc", ch );
        return;
    }
    if ( argument[0] == '\0' )
    {
	send_to_char( "IOoc what?\n\r", ch );
	return;
    }

      target = ECHOTAR_ALL;
      color = AT_IMMORT;
if( !strcmp(ch->name, "Shriak") )
{
  strcpy(arg,"&b-&c=&C{&GS&ghriak&C}&c=&b-&R: &w");
  strcat(arg, argument);
}
else if( !strcmp(ch->name, "Shiro") )
{
  strcpy(arg,"&R*&z&O)&B(&zShiro&B)&O(&R*&W:&z &w");
  strcat(arg, argument);
}
else if( !strcmp(ch->name, "Arcturus") )
{
  strcpy(arg, "&w&CA Gentleman:&G ");
  strcat(arg, argument);
}
else if( !strcmp(ch->name, "Vakshlad") )
{
  strcpy(arg, "&c-&.+&R{&p(&.Catwoman&p)&R}&.+&c-: ");
  strcat(arg, argument);
}
else 
{
  send_to_char("You aren't currently in the IOoc Database.  See Shiro or Arcturus to be put in.", ch);
  return;
}

    echo_to_all ( color, arg, target );
}
...
void do_infochan( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_IMMORTAL(ch) )
    {
        send_to_char( "Only imms may use this channel.", ch );
        return;
    }
    if ( argument[0] == '\0' )
    {
	send_to_char( "Inform the mud of what?\n\r", ch );
	return;
    }

      target = ECHOTAR_ALL;
      color = AT_IMMORT;
  strcpy( arg, "&R<::::::::::::&w=&YINFO&w=&R:::::::::::::> &G:&W ");
  strcat( arg, argument);
    echo_to_all ( color, arg, target );
}


void do_stupid( CHAR_DATA *ch, char *argument )
{
  char buf [MAX_STRING_LENGTH];
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  ...
  {
	victim->stupid = 1;
	ch_printf(victim, "You have earned the ire of the gods and have been accused of asking stupid questions by %s.\n\r", ch->name);
        sprintf( buf, "&Y%s&C has asked one too many stupid questions of the gods!", victim->name );
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL);
	save_char_obj(victim);	/* used to save ch, fixed by Thoric 09/17/96 */
  }
  else	
  {
	victim->stupid = 0;
	ch_printf(victim, "%s has decided you aren't as stupid as the gods had thought.\n\r", ch->name);
        sprintf( buf, "&Y%s&C is still an idiot, but no longer marked!", victim->name);
        echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL);
