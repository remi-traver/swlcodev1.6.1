...
void do_disabled (CHAR_DATA *ch, char *argument )
{
...
void do_sellhome( CHAR_DATA *ch, char *argument )
{
     ...
     if ( IS_SET( room->room_flags , ROOM_EMPTY_HOME ) )
     {
	send_to_char( "&RThis room is for buying only, go to another room....idoit!\n\r&w", ch);
	return;
     }

    if ( !IS_SET( room->room_flags , ROOM_PLR_HOME ) )
    {
	send_to_char( "&RYou must be in your home to sell it!\n\r", ch );
	return;
    }

     STRFREE( room->name );
     room->name = STRALLOC( "Empty Home" );
...
void do_lagout( CHAR_DATA *ch, char *argument )
{
   ...
   {
     send_to_char( "&RYou are NOT lagging out an immortal!\n\r", ch );
     return;
   }
	...
	send_to_char("Time for Mr. Bilbo Laggins to pay you a visit.\n\r", victim);
	WAIT_STATE(victim, x);
	send_to_char("Lag Applied!!!\n\r", ch);
	return;
}
/* Old clone coming back! - ezekiel 3/03
void do_clone( CHAR_DATA *ch, char *argument )
{
     ...
     if ( IS_NPC(ch) )
     {
       ch_printf( ch, "Look dumbass, you trying to crash the mud?\n\r" );
       return;
     }
     ...
     {
       ch->gold -= ch->top_level*0;
      
       ch_printf( ch, "&WA droid appears with a needle!\n\r" );
     }
     ...
     home = ch->plr_home;  
     ch->plr_home = NULL;
     status = ch->pcdata->status;
     ch->pcdata->status = 0;
     clanlevel = ch->pcdata->clanlevel;
     ch->pcdata->clanlevel = 0;
     qp = ch->pcdata->quest_curr;
     ...
     if ( ch->pcdata->clan_name && ch->pcdata->clan_name[0] != '\0' )
     {
       ...
       save_clone( ch );
     ch->plr_home = home;
     ch->pcdata->status = status;
     ch->gold = credits;
     ch->pcdata->bank = bank;
     ch->act = flags;
     ch->pcdata->clanlevel = clanlevel;
     ch->pcdata->quest_curr = qp;    
         
     ch_printf( ch, "&WIt takes a sample of your blood to the cloning labs!\n\r" );
...
void do_clone( CHAR_DATA *ch, char *argument )
{
     ...
     if ( (ch->in_room->vnum != 2137 )&&(ch->top_level < 101 && ch->top_level > 50))
     {
...
*/
       ...
       ch_printf( ch, "&WPoof!  you are cloned!\n");
       ch_printf( ch, "&WI wonder if they can do that with sheep?\n");
       ch_printf( ch, "&WYou are escorted into a small room.\n\r\n\r" );
     
     if (!IS_IMMORTAL(ch) && !IS_NEWBIE(ch))
     {  
       char_from_room( ch );
       char_to_room( ch, get_room_index( 2130 ) );
     }    
     flags   = ch->act;
     REMOVE_BIT( ch->act, PLR_KILLER );
     credits = ch->gold;
     ...
     {  
     char_from_room( ch );
     char_to_room( ch, get_room_index( 2138 ) );     
     do_look( ch , "" );
     }
...
void do_arm( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_NPC(ch) &&  ch->pcdata->learned[gsn_grenades] <= 0 )
    {
      ch_printf( ch, "You have no idea how to do that.\n\r" );
      return;
    }
    chance = IS_NPC(ch) ? get_trust(ch) 
                        : ch->pcdata->learned[gsn_grenades]; 
    ...
    {
       ch_printf(ch, "You can't seem to arm the device properly.\n\r" );
       learn_from_failure(ch, gsn_grenades);
       return;
    }
    obj->timer = 1;
    if ( obj->armed_by)
    ...
    sprintf( logbuf , "%s has armed %s in room room %d." , ch->name, obj->short_descr, ch->in_room->vnum);
    log_string( logbuf );
...
void do_ammo( CHAR_DATA *ch, char *argument )
{
    ...
    else if ( wield->value[3] == WEAPON_SLUGTHROWER )
    {
    
      if ( obj && obj->item_type != ITEM_SLUGAMMO )
      {
        send_to_char( "&RYour hands are too full to reload your slugthrower.\n\r&w", ch);
        return;
      }
      if (obj)
      {
        if ( obj->value[0] != wield->value[5] )   //check the amount of ammunition in the clip, and see if it's more than the gun can hold
        {
            send_to_char( "That clip is the wrong size for your gun.", ch);
            return;
        }
        if ( obj->value[1] != wield->value[2] )
        {
              send_to_char( "The ammunition is the wrong calibur!", ch);
              return;
        }
        unequip_char( ch, obj );
        checkammo = TRUE;
      ...
      else
      {
        for ( obj = ch->last_carrying; obj; obj = obj->prev_content )
        {
           if ( obj->item_type == ITEM_SLUGAMMO)
           {
             if ( obj->value[0] > wield->value[5] )   //check the amount of ammunition in the clip, and see if it's more than the gun can hold
             {
                 send_to_char( "That clip is too big for your gun.", ch);
                 return;
             }
             if ( obj->value[1] != wield->value[2] )  //check the calibur of the ammo and gun match.
             {
                 send_to_char( "The ammunition is the wrong calibur!", ch);
                 return;
             }
      ...
      wield->value[4] = charge;
      ch_printf( ch, "You discard the old clip, and slap a new one in place.\n\r", charge/5, charge );
      act( AT_PLAIN, "$n tosses the old clip away, and slaps a new one into $p.", ch, wield, NULL, TO_ROOM );
    ...
    else  if ( wield->value[3] == WEAPON_FLAMETHROWER )
    {

      if ( obj && obj->item_type != ITEM_FUELCELL )
      {
        send_to_char( "&RYour hands are too full to reload your flamethrower.\n\r&w", ch);
        return;
      }
      ...
      if (obj)
      {
        if ( obj->value[0] > wield->value[5] )
        {
            send_to_char( "That fuel cell is too large to fit into your flamethrower's fuel tank.", ch);
            return;
        }
        unequip_char( ch, obj );
        checkammo = TRUE;
      ...
      else
      {
        for ( obj = ch->last_carrying; obj; obj = obj->prev_content )
        {
           if ( obj->item_type == ITEM_FUELCELL)
           if ( obj->item_type == ITEM_FUELCELL)
           {
                 if ( obj->value[0] > wield->value[5] )
                 {
      ...
      if (!checkammo)
      {
        send_to_char( "&RYou don't seem to have any fuel cells to reload your flamethrower with.\n\r&w", ch);
        return;
      }
      wield->value[4] = charge;
      ch_printf( ch, "You fill up your flamethrower's fuel tank.\n\rThere is now %d units of fuel loaded.", charge );
      act( AT_PLAIN, "$n fills the fuel tank in $p.", ch, wield, NULL, TO_ROOM );
    }
    else
    {
	 ...
	 act( AT_PLAIN, "$p ignites with a bright glow.", ch, wield, NULL, TO_ROOM );
         sound_to_room( ch->in_room, "!!SOUND(wsaber)" );
         extract_obj( obj );
         wield->value[4] = charge;
      }
      ...
      {
         ch_printf( ch, "You replace the power cell in the shaft.\n\rYour axe begins to vibrate and is charged to %d/%d units.\n\r", charge, charge);
         act( AT_PLAIN, "$n replaces the power cell in $p.", ch, wield, NULL, TO_ROOM );
         extract_obj( obj );
         wield->value[4] = charge;
      }
      else if( wield->value[3] == WEAPON_VIBRO_SWORD )
      {
         ch_printf(ch, "You replace the power cell in the hilt.\n\rYour sword begins to vibrate and is charged to %d/%d units\n\r", charge, charge );
         act( AT_PLAIN, "$n replaces the power cell in $p.", ch, wield, NULL, TO_ROOM );
         extract_obj( obj );
         wield->value[4] = charge;
      }
...
void do_takedrug( CHAR_DATA *ch, char *argument )
{
    ...
    if(ch->race == RACE_DROID && !IS_IMMORTAL(ch))
    {
        send_to_char("Droids can't take drugs.\n\r", ch);
        return;
    }
...
void sith_penalty( CHAR_DATA *ch )
{
   ...
   { 
        ch->max_mana++ ;
   	send_to_char("&zYour power in the dark side grows.&w\n\r",ch);
   }
}
...
void do_drink( CHAR_DATA *ch, char *argument )
{
    ...
    if(ch->race == RACE_DROID && !IS_IMMORTAL(ch))
    {
       send_to_char("Droids can't drink things.\n\r", ch);
       return;
    }
...
//choke & puke on saltwater :P  Raijen, 9-21-01	

	if ( obj->value[2] == 14 )
	{
	   act( AT_ACTION, "$n gags and chokes on the saltwater, and proceeds to empty the contents of their stomach in the corner",
		ch, obj, liq_table[liquid].liq_name, TO_ROOM );
	   act( AT_ACTION, "You choke and gag onthe saltwater!.  You puke all over the corner!",
		ch, obj, liq_table[liquid].liq_name, TO_CHAR );
	gain_condition( ch, COND_FULL, -100 );
...
void do_quaff( CHAR_DATA *ch, char *argument )
{
    ...
    if ( obj->item_type != ITEM_POTION && obj->item_type != ITEM_ELIXIR)
    {
     ...
     */
/* Decided against it, its a potion or elixir, not food. -Arcturus
    if ( !IS_NPC(ch)
...
*/
    ...
    {
        send_to_char( "How do you expect that to work, you've got no stomach.\n\r", ch);
        return;
    }
    separate_obj( obj );
    if ( obj->in_obj )
	...
	gain_condition( ch, COND_THIRST, 1 );
	   ...
	   if ( retcode == rNONE )
        ...
        else if(obj->item_type == ITEM_ELIXIR )
        {
            retcode = rNONE;
            switch(obj->value[0])
            {
            default: break;
            case MEDICINE_HP: ch->hit += obj->value[1]; 
               ch_printf(ch, "That feels much better, you feel healed %d points of health.\n\r", obj->value[1]); 
               if(ch->hit > ch->max_hit)
                  ch->hit = ch->max_hit;
               break;
            case MEDICINE_MV: ch->move += obj->value[1];
               ch_printf(ch, "That feels much better, you feel rejuvenated %d points of movement.\n\r", obj->value[1]);
               if(ch->move > ch->max_move)
                  ch->move = ch->max_move;
               break;
            case MEDICINE_POISON: retcode = obj_cast_spell(skill_lookup("cure poison"), obj->value[1], ch, ch, NULL); break;
            case MEDICINE_BLINDNESS: retcode = obj_cast_spell(skill_lookup("cure blindness"), obj->value[1], ch, ch, NULL); break;
            case MEDICINE_CBLIND: retcode = obj_cast_spell( skill_lookup("blindness"), obj->value[1], ch, ch, NULL); break;
            case MEDICINE_CPOISON: retcode = obj_cast_spell( skill_lookup("poison"), obj->value[1], ch, ch, NULL); break;
            case MEDICINE_SLEEP: retcode = obj_cast_spell( skill_lookup("sleep"), obj->value[1], ch, ch, NULL); break;
            }
           if(retcode == rNONE)
           {
            switch(obj->value[2])
            {
            default: break;
            case MEDICINE_HP: ch->hit += obj->value[3]; 
               ch_printf(ch, "That feels much better, you feel healed %d points of health.\n\r", obj->value[3]);
               if(ch->hit > ch->max_hit)
                  ch->hit = ch->max_hit;
               break;
            case MEDICINE_MV: ch->move += obj->value[3];
               ch_printf(ch, "That feels much better, you feel rejuvenated %d points of movement.\n\r", obj->value[3]);
               if(ch->move > ch->max_move)
                  ch->move = ch->max_move;
               break;
            case MEDICINE_POISON: retcode = obj_cast_spell(skill_lookup("cure poison"), obj->value[3], ch, ch, NULL); break;
            case MEDICINE_BLINDNESS: retcode = obj_cast_spell(skill_lookup("cure blindness"), obj->value[3], ch, ch, NULL);break;
            case MEDICINE_CBLIND: retcode = obj_cast_spell( skill_lookup("blindness"), obj->value[3], ch, ch, NULL); break;
            case MEDICINE_CPOISON: retcode = obj_cast_spell( skill_lookup("poison"), obj->value[3], ch, ch, NULL); break;
            case MEDICINE_SLEEP: retcode = obj_cast_spell( skill_lookup("sleep"), obj->value[3], ch, ch, NULL); break; 
            }
           }
        }
    }
...
void do_rdesc( CHAR_DATA *ch, char *argument ) //Charles Made This. (7/14/02)
{
   ROOM_INDEX_DATA *room;
   ...
   {
     send_to_char( "&r&RThat's Not Your Home!&w\n\r", ch );
     return;
   }
   if ( ch->plr_home == NULL )
   {
     send_to_char( "&r&RYou don't own a home!&w\n\r", ch );
     return;
   }
   if ( ch->plr_home->vnum == ch->in_room->vnum )
   {
     do_redit( ch, "desc" );
     fold_area( room->area, room->area->filename, FALSE );
   }
...
void do_tamp( CHAR_DATA *ch, char *argument )
{
    ...
    if ( (pipe = get_obj_here( ch, arg )) == NULL )
    {
...
void do_smoke( CHAR_DATA *ch, char *argument )
{
    ...
    if( ch->race == RACE_DROID)
    {
        send_to_char("Smoke, but you are a droid.\n\r", ch);
        return;
    }
    if ( ms_find_obj(ch) )
	return;
...
void do_apply( CHAR_DATA *ch, char *argument )
{
    ...
    WAIT_STATE( ch, obj->value[2] + PULSE_VIOLENCE );
...
void do_hail( CHAR_DATA *ch , char *argument )
{
    int vnum;
    ROOM_INDEX_DATA *room;
    ROOM_INDEX_DATA *ex_room; /* Extra Room for swapping purpose. Used for finding the closet vnum to which to hail. */
    if ( !ch->in_room )
       return;
    ...
    {
       send_to_char( "Go outside first maybe?\n\r", ch );
       return;
    }

    if ( IS_SET( ch->in_room->room_flags, ROOM_NO_RECALL))
    {
       send_to_char("Speeders don't come around here, you are on your own.\n\r", ch);
       return;
    }
    ...
    for ( vnum = ch->in_room->area->low_r_vnum  ;  vnum <= ch->in_room->area->hi_r_vnum  ;  vnum++ )
    {
	    ...
	    {/* Hotel Bit. */
             if ( IS_SET(room->room_flags , ROOM_HOTEL ) && !IS_SET(room->room_flags, ROOM_PLR_HOME)
                && !IS_SET(room->room_flags, ROOM_EMPTY_HOME) && !IS_SET(room->room_flags, ROOM_NO_RECALL))
                {
                   if(ex_room == NULL) /* If there was not a place before there is now. */
                     ex_room = room;
                   else /* If there was another place. */
                   { /* Is it a closer? By vnum # not actual location. Should be correct however. */
                      if(abs(ex_room->vnum - ch->in_room->vnum) > abs(room->vnum - ch->in_room->vnum) )
                         ex_room = room; /* If its closer thats where we want to go. */
                   }
                }       
	    }
    }
    ...
    if ( ex_room == NULL )
    {
       send_to_char( "There doesn't seem to be any speeders nearby!\n\r", ch );
       return;
    }
...
void do_train( CHAR_DATA *ch, char *argument )
{
    ...
    { 
    	default:
	        ...
	        && str_cmp( arg, "dex" ) && str_cmp( arg, "dexterity" )
                ...
                if ( !str_cmp( arg, "str" ) || !str_cmp( arg, "strength" ) )
                {
                      if( mob->perm_str <= ch->perm_str || ch->perm_str >= 20 + race_table[ch->race].str_plus || ch->perm_str >= 40 )
                      {
	    	...
	    	{
                      if( mob->perm_dex <= ch->perm_dex || ch->perm_dex >= 20 + race_table[ch->race].dex_plus || ch->perm_dex >= 40 )
                      {
    		...
    		{
                      if( mob->perm_int <= ch->perm_int || ch->perm_int >= 20 + race_table[ch->race].int_plus || ch->perm_int >= 40 )
                      {
                          act( AT_TELL, "$n tells you 'I cannot help you... you are already more educated than I.'",
		             mob, NULL, ch, TO_VICT );
		          return;
                      }
                      send_to_char("&GYou begin your studies in an effort to become educated.\n\r", ch);
                }
          	if ( !str_cmp( arg, "wis" ) || !str_cmp( arg, "wisdom" ) )
    		{
                      if( mob->perm_wis <= ch->perm_wis || ch->perm_wis >= 20 + race_table[ch->race].wis_plus || ch->perm_wis >= 40 )
                      {
                          act( AT_TELL, "$n tells you 'I cannot help you... you are already far wiser than I.'",
		             mob, NULL, ch, TO_VICT );
		          return;
                      }
                      send_to_char("&GYou begin contemplating several historical documents in an effort to gain wisdom.\n\r", ch);
                }
          	if ( !str_cmp( arg, "con" ) || !str_cmp( arg, "constitution" ) )
    		{
                      if( mob->perm_con <= ch->perm_con || ch->perm_con >= 20 + race_table[ch->race].con_plus || ch->perm_con >= 40 )
                      {
          	...
          	if ( !str_cmp( arg, "cha" ) || !str_cmp( arg, "charisma" ) )
            	{
                      if( mob->perm_cha <= ch->perm_cha || ch->perm_cha >= 20 + race_table[ch->race].cha_plus || ch->perm_cha >= 40 )
                      {
            	...
            	add_timer ( ch , TIMER_DO_FUN , 2 , do_train , 1 );
    ...
    if ( !str_cmp( arg, "wis" ) || !str_cmp( arg, "wisdom" ) )
    {
        if ( !successful )
        {
             send_to_char("&RStudying the historical documents has left you more confused than wise...\n\r", ch);
             return;	
        } 
    ...
    if ( !str_cmp( arg, "con" ) || !str_cmp( arg, "constitution" ) )
    {
        ...
        send_to_char("&GAfter a long tiring exercize session you feel much healthier than before.\n\r", ch);
    	ch->perm_con++;
    	return;
    }
...
void do_suicide( CHAR_DATA *ch, char *argument )
{
        ...
        if ( strcmp( crypt( argument, ch->pcdata->pwd ), ch->pcdata->pwd ) )
	{
	    ...
	    sprintf( logbuf , "%s attempting to commit suicide... WRONG PASSWORD!, What an idiot!!!" , ch->name );
	    log_string( logbuf );
	    return;
	}
       ...
       sprintf( buf, "%s is dead!!! (Suicide)", ch->name);
       echo_to_all( AT_RED, buf, 0);
       sprintf( logbuf, "%s: suicide", ch->name);
       log_string(logbuf);
       set_cur_char(ch);
       remroster(ch);
       raw_kill( ch, ch );
...
void do_bank( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
       send_to_char( "Usage: BANK <deposit|withdraw|balance|transfer> [amount] [person]\n\r", ch );
       return;
    }
    ...
    else if ( !str_prefix( arg1 , "withdraw" ) )
    {
       ...
       if ( ch->pcdata->bank < amount && !IS_lMMORTAL(ch))
       {
  ...
  if ( !str_cmp( arg1, "transfer" ) )
  {
    ...
    {
      send_to_char( "Please Use A Specific Amount\n\r", ch );
      return;
    }
    ...
    if ( amount <= 0 )
    {
      send_to_char( "Try Entering A Value Above 0\n\r", ch );
      return;
    }
    
    ch->pcdata->bank     -= amount;
    victim->pcdata->bank += amount;
    sprintf( buf, "You transfer %ld gold coin%s to %s's bank account.\n\r", amount, (amount != 1) ? "s" : "", victim->name );
    send_to_char( buf, ch );
    sprintf( buf, "%s just transferred %ld gold coin%s to your bank account.\n\r", ch->name, amount, (amount != 1) ? "s" : "" );
	...
	if ( ( victim = get_char_world( ch, arg3 ) ) == NULL )
       ...
       {
          send_to_char( "You don't have that many credits in the bank.\n\r", ch );
          return;
       }
	 ...
	 if ( amount == 1 )
	 {
           ch_printf( ch , "You deposit %ld credit into %s's account.\n\r" ,amount , victim->name );
	     ch_printf( victim , "%s deposits %ld credit into your account.\n\r" , ch->name , amount );
           return;
	 }

	 else
	 {
	     ch_printf( ch , "You deposit %ld credits into %s's account.\n\r" ,amount , victim->name );
	     ch_printf( victim , "%s deposits %ld credits into your account.\n\r" , ch->name , amount );
           return;
 	 }
    }
...
/* Parnic's NEW abdicate, Inserted by Raijen */
...
void do_gecko( CHAR_DATA *ch, char *argument )
{
    do_ooc(ch, "I'm a gecko! a gecko!!!!");
}    
