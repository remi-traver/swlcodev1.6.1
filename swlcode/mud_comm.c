...
void do_mpforce( CHAR_DATA *ch, char *argument )
{
	...
	for ( vch = ch->in_room->first_person; vch; vch = vch->next_in_room )
	    if ( get_trust( vch ) < get_trust( ch ) && can_see( ch, vch ) && !IS_SET(vch->act, PLR_AFK) )
...
/*
 * syntax: mprefresh (character) (#movement)                Nusco
 */
void do_mp_refresh( CHAR_DATA *ch, char *argument )
{
    char arg1[ MAX_INPUT_LENGTH ];
    char arg2[ MAX_INPUT_LENGTH ];
    CHAR_DATA *victim;
    ...
    if ( !IS_NPC( ch ) || ( ch->desc && get_trust( ch ) < LEVEL_IMMORTAL )  )
    {
        send_to_char( "Since when can YOU do that??\n\r", ch );
        return;
    }
    argument = one_argument( argument, arg1 );
    argument = one_argument( argument, arg2 );
    ...
    if ( arg1[0] == '\0' )
    {
       send_to_char( "mprefresh whom?\n\r", ch );
       progbug( "Mprefresh: invalid person", ch );
       return;
    }

    if ( arg2[0] == '\0' )
    {
       send_to_char( "mprefresh how much movement?\n\r", ch );
       progbug( "Mprefresh: invalid amount", ch );
       return;
    }
    ...
    {
        send_to_char( "Victim must be in room.\n\r", ch );
        progbug( "Mprefresh: victim not in room", ch );
        return;
    }

    move = atoi(arg2);

    if( (move<0) || (move>32000) )
    {
       send_to_char( "Mprefresh how much?\n\r", ch );
       progbug( "Mprefresh: invalid (nonexistent?) argument", ch );
       return;
    }
    move += victim->move;
    victim->move = (move > 32000 || move < 0 || move > victim->max_move) ?
...
//    progbug( "Mpaward: Pre ATOI", ch );
    qpgain = atoi(arg2);
//    progbug( "Mpaward: Ppst ATOI", ch );

    if( qpgain < 0 || qpgain > 10 )
    {
    progbug( "Mpaward:badrange", ch );

       send_to_char( "Mpaward how much (1-10)\n\r", ch );
       progbug( "Mpaward: QP Value out of range (1-10)", ch );
       return;
    }
...
//    if ( ch->gold < 1000000000 && gold < 1000000000 && ch->in_room    &&   economy_has( ch->in_room->area, gold ) )
    ...
    {
	ch->gold += gold;
//	lower_economy( ch->in_room->area, gold ); commented out to prevent bounty mobs from running out of money
    }
}
...
CHAR_DATA *get_char_room_mp( CHAR_DATA *ch, char *argument )
{
	...
	||  (IS_NPC(rch) && vnum == rch->pIndexData->vnum))
        ||  (!IS_NPC(rch) && nifty_is_name(arg, NAME(rch))) ) /* diguise bit? - Arcturus */
	{
	...
	if ( !nifty_is_name_prefix( arg, rch->name ) 
             || (!IS_NPC(rch) && !nifty_is_name_prefix(arg, NAME(rch))) ) /*Disguise bit? -Arcturus */
	    continue;
	if ( number == 0 && !IS_NPC(rch) )
	    return rch;
	else
