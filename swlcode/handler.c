...
void do_reset_age( CHAR_DATA *ch, char *argument);
...
sh_int get_age( CHAR_DATA *ch )
{
		...
		&& ch->pcdata->birthday.month <= -1 )
	{
		bug("%s doesn't have a Birthday",ch->name);
                bug("%s assigned birthday, age reset to 17.", ch->name);
	...
	/*years = ( (time_info.year - ch->pcdata->birthday.year) + ch->pcdata->age );*/
        /* Adapted to Starwars Months Fix */
        if( ch->pcdata->birthday.month >= 10)
        {
            ch->pcdata->birthday.month -= 10;
            bug("%s birthmonth no longer existant, fixed.", ch->name);
        }
	years = ( (time_info.year - ch->pcdata->birthday.year) + ch->pcdata->age );
	months = 10 + ( time_info.month - ch->pcdata->birthday.month );
	months %= 10;
	...
	if ( time_info.month < ch->pcdata->birthday.month )
        ...
        if( years > 200 )
        {
           bug("%s is insanely old, probably due to weather bug.\n\r", ch->name);
           bug("%s's age has been reset.\n\r", ch->name);
           ch->pcdata->birthday.year = time_info.year;
           ch->pcdata->birthday.month = time_info.month;
           ch->pcdata->birthday.day = time_info.day;
           ch->pcdata->age = 17;
...
sh_int get_curr_str( CHAR_DATA *ch )
{
    ...
    max  = 40;
//    max  = 40 + race_table[ch->race].str_plus;
    max = UMIN(max,40);
...
//    max  = 30 + race_table[ch->race].wis_plus;
    max  = 40;
    max = UMIN(max,40);
...
//    max  = 30 + race_table[ch->race].dex_plus;
    max  = 40;
    max = UMIN(max,40);
...
sh_int get_curr_con( CHAR_DATA *ch )
{
    sh_int max;
   max = 40;    
//    max  = 30 + race_table[ch->race].con_plus;
    max = UMIN(max,40);
...
//    max  = 30 + race_table[ch->race].cha_plus;
...
sh_int get_curr_lck( CHAR_DATA *ch )
{
    ...
    max  = 30 + race_table[ch->race].lck_plus;
    max = UMIN(max,40);
...
sh_int get_curr_frc( CHAR_DATA *ch )
{
    ...
    max  = 30 + race_table[ch->race].frc_plus;
    max = UMIN(max,40);
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
    ...
    case APPLY_WEIGHT:	      ch->carry_weight		+= mod; break;
...
//    case APPLY_EVADE:	      ch->evade			+= mod; break;
//    case APPLY_LEECH:	      ch->leech			+= mod; break;
    case APPLY_MAGIC_RESISTANCE: ch->magic_resistance	+= mod; break;
    case APPLY_DAMAGE_RESISTANCE: ch->damage_resistance	+= mod; break;
    case APPLY_SEX:
	ch->sex = (ch->sex+mod) % 3;
	if ( ch->sex < 0 )
	  ch->sex += 2;
...
//    case APPLY_DAMCAP:        ch->damcap		+= mod;	break;
    case APPLY_SNIPERWEAPON:  ch->sniperweapon		+= mod;	break;
    case APPLY_MOVE:          ch->max_move		+= mod;	break;
    case APPLY_GOLD:						break;
    case APPLY_EXP:						break;
    case APPLY_ATTACK_SPEED:						break;
    case APPLY_AC:            ch->armor			+= mod;	break;
    case APPLY_NUM_ATTACKS:            ch->bonus_attacks		+= mod;	break;
	...
	if ( !IS_NPC(ch) )
		...
		URANGE( 0, ch->pcdata->condition[COND_FULL] + mod, 49 );
	break;

    case APPLY_THIRST:
	if ( !IS_NPC(ch) )
	    ch->pcdata->condition[COND_THIRST] =
		URANGE( 0, ch->pcdata->condition[COND_THIRST] + mod, 49 );
	break;

    case APPLY_DRUNK:
	if ( !IS_NPC(ch) )
	    ch->pcdata->condition[COND_DRUNK] =
		URANGE( 0, ch->pcdata->condition[COND_DRUNK] + mod, 49 );
	break;
...
/* skill apply types	-Thoric */
    ...
    case APPLY_PALM:
        if( !IS_NPC(ch) && ch->pcdata->learned[gsn_palm] > 0 )
          ch->pcdata->learned[gsn_palm] = UMAX( 1, ch->pcdata->learned[gsn_palm] + mod);
        break;
    case APPLY_TRACK:
    ...
    case APPLY_NEGOTIATE:
        if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_negotiate]> 0)
          ch->pcdata->learned[gsn_negotiate] =
                UMAX( 1, ch->pcdata->learned[gsn_negotiate] + mod );
        break;
    case APPLY_KICK:
	...
	break;
    case APPLY_VIBRO:
	if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_vibro_blades]> 0)
	  ch->pcdata->learned[gsn_vibro_blades] =
		UMAX( 1, ch->pcdata->learned[gsn_vibro_blades] + mod );
	break;
    case APPLY_BLASTERS:
	if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_blasters]> 0)
	  ch->pcdata->learned[gsn_blasters] =
		UMAX( 1, ch->pcdata->learned[gsn_blasters] + mod );
	break;
    case APPLY_SWORDS:
	if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_swords]> 0)
	  ch->pcdata->learned[gsn_swords] =
		UMAX( 1, ch->pcdata->learned[gsn_swords] + mod );
	break;
    case APPLY_LIGHTSABERS:
	if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_lightsabers]> 0)
	  ch->pcdata->learned[gsn_lightsabers] =
		UMAX( 1, ch->pcdata->learned[gsn_lightsabers] + mod );
	break;
    case APPLY_UNARMED:
	if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_unarmed]> 0)
	  ch->pcdata->learned[gsn_unarmed] =
		UMAX( 1, ch->pcdata->learned[gsn_unarmed] + mod );
	break;
    case APPLY_WHIPS:
       if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_whip]> 0)
          ch->pcdata->learned[gsn_whip] =
                UMAX( 1, ch->pcdata->learned[gsn_whip] + mod );
       break;
    case APPLY_BLUDGEONS:
       if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_bludgeons]> 0)
          ch->pcdata->learned[gsn_bludgeons] =
                UMAX( 1, ch->pcdata->learned[gsn_bludgeons] + mod );
       break;
    case APPLY_AXES:
       if( !IS_NPC(ch) && ch->pcdata->learned[gsn_vibroaxes] > 0)
          ch->pcdata->learned[gsn_vibroaxes] = UMAX( 1, ch->pcdata->learned[gsn_vibroaxes] + mod);
       break;
    case APPLY_FLAMETHROWERS:
       if( !IS_NPC(ch) && ch->pcdata->learned[gsn_flamethrowers] > 0)
          ch->pcdata->learned[gsn_flamethrowers] = UMAX( 1, ch->pcdata->learned[gsn_flamethrowers] + mod);
       break;
    case APPLY_BOWCASTERS:
       if( !IS_NPC(ch) && ch->pcdata->learned[gsn_bowcasters] > 0 )
          ch->pcdata->learned[gsn_bowcasters] = UMAX( 1, ch->pcdata->learned[gsn_bowcasters] + mod);
       break;
    case APPLY_SLUGTHROWERS:
       if( !IS_NPC(ch) && ch->pcdata->learned[gsn_slugthrowers] > 0)
          ch->pcdata->learned[gsn_slugthrowers] = UMAX( 1, ch->pcdata->learned[gsn_slugthrowers] + mod);
       break;
    case APPLY_FORCEPIKES:
       if( !IS_NPC(ch) && ch->pcdata->learned[gsn_force_pikes] > 0)
          ch->pcdata->learned[gsn_force_pikes] = UMAX( 1, ch->pcdata->learned[gsn_force_pikes] + mod);
       break;
    case APPLY_PICKSHIPLOCK:
       if( !IS_NPC(ch) && ch->pcdata->learned[gsn_pickshiplock] > 0)
          ch->pcdata->learned[gsn_pickshiplock] = UMAX(1, ch->pcdata->learned[gsn_pickshiplock] + mod);
       break;
    case APPLY_BREW:
       if( !IS_NPC(ch) && ch->pcdata->learned[gsn_brew] > 0)
          ch->pcdata->learned[gsn_brew] = UMAX(1, ch->pcdata->learned[gsn_brew] + mod);
       break;
    case APPLY_EVADE:
       if( !IS_NPC(ch) && ch->pcdata->learned[gsn_evade] > 0 )
          ch->pcdata->learned[gsn_evade] = UMAX(1, ch->pcdata->learned[gsn_evade] + mod);
       break;
    case APPLY_TUCK:
       if( !IS_NPC(ch) && ch->pcdata->learned[gsn_tuck] > 0)
          ch->pcdata->learned[gsn_tuck] = UMAX(1, ch->pcdata->learned[gsn_tuck] + mod );
       break;
    }
...
{
    if ( obj->item_type != ITEM_ARMOR && obj->item_type != ITEM_CARMOR)
	return 0;
    ...
    case WEAR_TRINKET:  return	   obj->value[0];
    }
...
CHAR_DATA *get_char_room( CHAR_DATA *ch, char *argument )
{
	...
	&& ( nifty_is_name( arg, rch->name )
        || (!IS_NPC(rch) && nifty_is_name(arg, NAME(rch))) /* Disguise? */
...
/*	if ( !can_see( ch, rch ) || !nifty_is_name_prefix( arg, rch->name )
            || (IS_NPC(rch) && !nifty_is_name_prefix(arg, NAME(rch))) )
*/
        if( !can_see(ch, rch) || !(IS_NPC(rch) ? nifty_is_name_prefix( arg, rch->name )
                                              : nifty_is_name_prefix( arg, NAME(rch) ) 
...
CHAR_DATA *get_char_world( CHAR_DATA *ch, char *argument )
{
	...
	if ( (nifty_is_name( arg, wch->name )
        ||  (!IS_NPC(wch) && nifty_is_name(arg, NAME(wch))) /*Disguise Bit? - Arcturus */
...
/*	if ( !nifty_is_name_prefix( arg, wch->name ) 
             || (IS_NPC(wch) && !nifty_is_name_prefix(arg, NAME(wch))) ) */
        if( !(IS_NPC(wch) ? nifty_is_name_prefix( arg, wch->name )
                                              : nifty_is_name_prefix( arg, NAME(wch) )
...
*/
	...
	&&   !victim->fighting 
	&& !(ch->race == RACE_DUINUOGWUIN && GET_AGE(ch) >= 75))
	  return FALSE;
...
char *affect_loc_name( int location )
{
    ...
    case APPLY_SNIPERWEAPON:	return "Sniper Rifle";
    case APPLY_LEECH:		return "Leech Remove Me";
    case APPLY_EVADE:		return "Evade";
    ...
    case APPLY_VIBRO:		return "vibro-blades";
    case APPLY_BLASTERS:	return "blasters";
    case APPLY_SWORDS:		return "swords";
    case APPLY_LIGHTSABERS:	return "lightsabers";
    case APPLY_UNARMED:		return "unarmed combat";
	...
	case APPLY_LASERDAMAGE:	return "laser damage";
	case APPLY_SALVO:	return "salvo system";
	case APPLY_MAGIC_RESISTANCE:	return "magic resistance remove me";
	case APPLY_DAMAGE_RESISTANCE:	return "damage resistance";
	case APPLY_ATTACK_SPEED:	return "attack speed";
	case APPLY_NUM_ATTACKS:	return "number of attacks";
        case APPLY_IONS:        return "ions";
        case APPLY_ENGINE:      return "engine";
        case APPLY_GENERATOR: return "shield generator";
        case APPLY_HASSEMBLY: return "hyperdrive assembly";
        case APPLY_OVERDRIVE: return "SLAM overdrive node";
        case APPLY_LASERBATTERY: return "laser battery";
        case APPLY_ARMOR: return "ship armor";
        case APPLY_CARGO: return "cargo space";
        case APPLY_MLAUNCHER: return "missile launcher";
        case APPLY_TLAUNCHER: return "torpedo launcher";
        case APPLY_RLAUNCHER: return "rocket launcher";
        case APPLY_SIMULATOR: return "ship simulation moderator";
        case APPLY_WHIPS:     return "whips";
        case APPLY_BLUDGEONS: return "bludgeons";
        case APPLY_CLOAK:     return "cloaking device";
        case APPLY_AXES:      return "axes";
        case APPLY_FLAMETHROWERS: return "flamethrowers";
        case APPLY_BOWCASTERS:    return "bowcasters";
        case APPLY_SLUGTHROWERS:  return "slugthrowers";
        case APPLY_FORCEPIKES:    return "force pikes";
        case APPLY_PICKSHIPLOCK:  return "pick ship lock";
        case APPLY_TUCK:          return "tuck";
    }
...
//    if ( vector & AFF_FAERIE_FIRE   ) strcat( buf, " faerie_fire"   );
    if ( vector & AFF_MASKED_AURA     ) strcat( buf, " masked_aura"   );
    if ( vector & AFF_INFRARED      ) strcat( buf, " infrared"      );
//    if ( vector & AFF_CURSE         ) strcat( buf, " curse"         );
    if ( vector & AFF_TRACING         ) strcat( buf, " tracing"      );
    if ( vector & AFF_REGEN         ) strcat( buf, " regeneration"  );
    ...
    if ( vector & AFF_DIPIMMUNITY   ) strcat( buf, " dip_immunity"  );
    if ( vector & AFF_SIT_AWARE     ) strcat( buf, " sit. awareness");
    if ( vector & AFF_POSSESS       ) strcat( buf, " possess"       );
    if ( vector & AFF_BERSERK       ) strcat( buf, " berserk"       );
    if ( vector & AFF_AQUA_BREATH   ) strcat( buf, " aqua_breath"   );
    if ( vector & AFF_SILVERTONGUE  ) strcat( buf, " silvertongue"     );
    if ( vector & AFF_ICESHIELD     ) strcat( buf, " remove_me"     );
//    if ( vector & TRACE_COMLINK ) strcat( buf, " trace comlink" );
...
char *extra_bit_name( int extra_flags )
{
    ...
    if ( extra_flags & ITEM_TWO_HANDED   ) strcat( buf, " two handed"   );
    ...
    if ( extra_flags & ITEM_DEATHROT   ) strcat( buf, " rotting"	);
    ...
    if ( extra_flags & ITEM_DEMONSTEEL     ) strcat( buf, " demonsteel"     );
    if ( extra_flags & ITEM_ANGELSTEEL   ) strcat( buf, " angelsteel"         );
    if ( extra_flags & ITEM_RELIC 	 ) strcat( buf, " Aincent Relic" );
...
{
    OBJ_DATA *check;
    if(obj->item_type == ITEM_TRAP) //Test to see if obj type trap can
      return obj; // be caught without using reset traps for first_content etc.
    if ( !obj->first_content)
      return NULL;
...
void do_reset_age( CHAR_DATA *ch, char *argument )
{
    ...
    if ( (!IS_NPC(victim)
    && ( get_trust( ch ) < 103 || get_trust( ch ) < get_trust( victim ) )))
    {
        send_to_char( "You failed.\n\r", ch );
        return;
    }
    victim->pcdata->birthday.year = time_info.year;
    victim->pcdata->birthday.month = time_info.month;
    victim->pcdata->birthday.day = time_info.day;
    victim->pcdata->age = 17;
    send_to_char("Ok.\n\r", ch);
    send_to_char("Your age has been reset. You feel very youthful.\n\r", victim);
    return;
}
