...
//	learn_from_failure( ch, sn );
	...
	/**********
	** Raijen's Handy MR Code
	**********
		if ( (skill->target != TAR_CHAR_DEFENSIVE
		||    skill->target != TAR_CHAR_SELF)
	        && victim->magic_resistance > 0 )
	{
	  if ( victim->magic_resistance > 95 )
	    temp_mr = 95;
	  else 
	    temp_mr = victim->magic_resistance;
	  if ( number_percent() < temp_mr )		
	  {
	...
	***/
	 ...
	 */
		...
		else
		{
		   ...
		   retcode = (*skill->spell_fun) ( sn, ch->skill_level[skill->guild], ch, vo );
		   end_timer(&time_used);
    ...
    if ( retcode != rSPELL_FAILED )
    {
        ...
        force_exp = URANGE( 0 , force_exp, ( exp_level(ch->skill_level[skill->guild]+1 )-exp_level(ch->skill_level[skill->guild] ) )/35 );
	if( !ch->fighting  )
	  ch_printf( ch, "You gain %d force experience.\n\r" , force_exp );
	gain_exp(ch, force_exp, skill->guild );
...
ch_ret spell_change_sex( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    do
    {
	af.modifier  = abs(number_range( 0, 2 ) - victim->sex);
    }
    while ( af.modifier == 0 );
    af.bitvector = 0;
    affect_to_char( victim, &af );
...
ch_ret spell_dispel_evil( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( saves_spell_staff( level, victim ) )
    dam /= 2;
    dam = dam * ( 1 + (abs(victim->alignment) /2500));
    return damage( ch, victim, dam, sn );
}
...
ch_ret spell_frostball( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    static const sh_int dam_each[] =
    {
...
ch_ret spell_identify( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( IS_OBJ_STAT( obj, ITEM_GLOW) )
		ch_printf( ch, "It is glowing with a mystic light.\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_HUM) )
		ch_printf( ch, "It is humming by itself.\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_TWO_HANDED) )
		ch_printf( ch, "It is to awkward to use with one hand\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_CONTRABAND) )
		ch_printf( ch, "It is a smuggled item\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_BLESS) )
		ch_printf( ch, "It is protected by the force\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_MAGIC) )
		ch_printf( ch, "It is imbued with the power of the force\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_NODROP) )
		ch_printf( ch, "It cannot be dropped\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_NOREMOVE) )
		ch_printf( ch, "It cannot be taken off\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_POISONED) )
		ch_printf( ch, "It is poisoned\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_DEMONSTEEL) )
		ch_printf( ch, "It is forged of the blackest Demonsteel\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_ANGELSTEEL) )
		ch_printf( ch, "It is forged of the brightest Angelsteel\n\r");
//	if ( IS_OBJ_STAT( obj, ITEM_ANTIMORTAL) )
//		ch_printf( ch, "It is not meant for mortal hands\n\r");
//	if ( IS_OBJ_STAT( obj, ITEM_SOULSTEALER) )
//		ch_printf( ch, "It steals the souls of it's foes\n\r");
//	if ( IS_OBJ_STAT( obj, ITEM_ANTISABER) )
//		ch_printf( ch, "It is designed to drain a Lightsabers power when struck\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_ANTI_EVIL) )
		ch_printf( ch, "It is to pure to be wielded by those of evil alignment\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_ANTI_GOOD) )
		ch_printf( ch, "It disdains those of pure heart\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_ANTI_NEUTRAL) )
		ch_printf( ch, "It is opposed to those with indicisive hearts\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_SMALL_SIZE) )
		ch_printf( ch, "It is made for smaller than average sized people.\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_HUMAN_SIZE) )
		ch_printf( ch, "It is made for average sized people.\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_LARGE_SIZE) )
		ch_printf( ch, "It is made for people that are larger than average.\n\r");
	if ( IS_OBJ_STAT( obj, ITEM_HUTT_SIZE) )
		ch_printf( ch, "It is made for people as big as a hutt.\n\r");
    ...
    case ITEM_WEAPON:
        switch(obj->value[3])
        {
        case WEAPON_NONE: ch_printf(ch, "This appears to be an unarmed weapon.\n\r"); break;
	...
	else if ( obj->value[3] == WEAPON_BOWCASTER )
	{
	   ch_printf( ch, "It has %d out of %d energy bolts remaining.\n\r",
        ...
        {
           ch_printf( ch, "It has %d out of %d, fuel cells remaining.\n\r", obj->value[4], obj->value[5] );
        }
        else if (obj->value[3] == WEAPON_SLUGTHROWER )
        {
           ch_printf( ch, "It has %d out of %d, rounds remaining.\n\r", obj->value[4], obj->value[5] );
    ...
    case ITEM_FIGHTERCOMP:
        ch_printf( ch, "It was designed for starfighters.\n\r");
        break;
    case ITEM_MIDCOMP:
        ch_printf( ch, "It was designed for midships.\n\r" );
        break;
    case ITEM_FRIGATECOMP:
        ch_printf( ch, "It was designed for frigates.\n\r" );
        break;
    case ITEM_CAPITALCOMP:
        ch_printf(ch, "It was designed for capital class ships.\n\r" );
        break;
    case ITEM_SCAPITALCOMP:
        ch_printf(ch, "It was designed for super capital class ships.\n\r" );
        break;
    }
    ...
    if ( chance(ch, 50)    ||    IS_IMMORTAL(ch) )
    {
...
ch_ret spell_poison( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( chance == 1000 || saves_poison_death( chance, victim ) || victim->race == RACE_DROID )
	return rSPELL_FAILED;
...
ch_ret spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    for ( ;; )
    {
	pRoomIndex = get_room_index( number_range( 0, 104857600 ) );
	if ( pRoomIndex )
...
ch_ret spell_gas_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
	set_char_color( AT_MAGIC, ch );
	send_to_char( "You fail to create the cloud.\n\r", ch );
	return rNONE;
    }
...
ch_ret spell_major_scourge( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *vch;
    CHAR_DATA *vch_next;
    int dam;
	...
	vch_next = vch->next_in_room;
      if ( !IS_NPC( vch ) && IS_SET( vch->act, PLR_WIZINVIS ) && vch->pcdata->wizinvis >= LEVEL_IMMORTAL )
          continue;
      if ( !is_same_group(ch, vch) && is_legal_kill(ch, vch) && can_see(ch, vch) && !is_safe(ch, vch) && ch != vch)
	{
	    hpch = UMAX( 10, ch->hit );
...
ch_ret spell_research( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    /* This skill was created by spec. Fuck off. */
    if ( ( victim = get_char_world( ch, target_name ) ) == NULL
    ||   victim == ch
    ||   !victim->in_room
    ||   victim->top_level >= level + 15
...
{
        send_to_char( "Your victim resists the probe\n\r", ch );
	send_to_char("You feel as though somoene's looking for you...\n\r", victim);
	failed_casting( skill, ch, victim, NULL );
	return rSPELL_FAILED;
}
    ...
    {
	    send_to_char("Your target's mind is FAR to fortified to find.\n\r", ch);
	return rSPELL_FAILED;
    }
    ...
    if ( IS_IMMORTAL(victim) && (victim->top_level > ch->top_level ) )
{
    ...
    if ( number_percent()  > (ch->top_level - (victim->top_level / 2 ) ) )
    {
	    send_to_char("You cannot focus on your target.\n\r", ch);
	send_to_char("You feel as though somoene's looking for you...\n\r", victim);
	return rSPELL_FAILED;
    }


	    send_to_char("You can feel someone's eyes on you...\n\r", victim);
...
ch_ret spell_recharge( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( chance(ch, 50 - (ch->skill_level[ALTER_ABILITY]) ) )
	{
...
ch_ret spell_remove_invis( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if( !IS_NPC(victim) )
	{
	  if( chance(ch, 50) && ch->skill_level[ALTER_ABILITY] < victim->top_level )
	  {
	     failed_casting( skill, ch, victim, NULL );
	     return rSPELL_FAILED;
	  }
	...
	{
	  if( chance(ch, 50) && ch->skill_level[ALTER_ABILITY] + 15 < victim->top_level )
	  {
	     failed_casting( skill, ch, victim, NULL );
	     return rSPELL_FAILED;
	  }
...
ch_ret spell_animate_dead( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( !IS_NPC(ch) )
    {
      ...
      {
  	send_to_char("You do not have enough force power to reanimate this "
  	            "corpse.\n\r", ch);
	return rSPELL_FAILED;
      }
      else
	...
	act(AT_MAGIC, "$n imbues $T with the Darkside and makes $T rise from the grave!", ch, NULL, pMobIndex->short_descr, TO_ROOM);
	act(AT_MAGIC, "You imbue $T with the Darkside and make $T rise from the grave!", ch, NULL, pMobIndex->short_descr, TO_CHAR);
...
ch_ret spell_possess( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.type      = sn;
    af.duration  = 20 + (ch->skill_level[ALTER_ABILITY] - victim->top_level);
    af.location  = 0;
    af.modifier  = 0;
...
ch_ret spell_disabled( int sn, int level, CHAR_DATA *ch, void *vo )
{
        send_to_char( "&RThis spell is currently disabled!!!\n\r&w", ch);
        return;
}
...
ch_ret spell_obj_inv( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	case SA_CREATE:
	  ...
	  switch( SPELL_POWER(skill) )		/* clone object */
	  {
	     ...
	     default:
	     case SP_NONE:
		if ( obj->cost > ch->skill_level[skill->guild] * get_curr_int(ch) * get_curr_wis(ch) )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
		break;
	     case SP_MINOR:
		if ( ch->skill_level[skill->guild] - obj->level < 20
		||   obj->cost > ch->skill_level[skill->guild] * get_curr_int(ch) / 5 )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
		break;
	     case SP_GREATER:
		if ( ch->skill_level[skill->guild] - obj->level < 5
		||   obj->cost > ch->skill_level[skill->guild] * 10 * get_curr_int(ch) * get_curr_wis(ch) )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
		break;
	     case SP_MAJOR:
		if ( ch->skill_level[skill->guild] - obj->level < 0
		||   obj->cost > ch->skill_level[skill->guild] * 50 * get_curr_int(ch) * get_curr_wis(ch) )
		{
		   failed_casting( skill, ch, NULL, obj );
		   return rNONE;
		}
...
ch_ret spell_black_lightning( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    dam         = 100 + ( 100 * ( 1 + ( number_percent( ) /100 ) ) );
